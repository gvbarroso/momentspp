...
with open("bench_params.csv") as fin:
    params = fin.readlines()

p_names = params[0].strip().split(",")

with open("bench_python.csv", "w+") as fout:
    ...
    for p_list in params[1:]:
        ...
        N0 = eval(p_list[p_names.index('Ne_0')])
        N1 = eval(p_list[p_names.index('Ne_1')])
        rho = 4 * N0 * r
        theta = 4 * N0 * u
        nus = [N0 / N0, N1 / N0]
        ...
        if num_epochs == 2:
            T = eval(p_list[p_names.index('num_gen_epoch_2')]) / 2 / N0
            N0b = eval(p_list[p_names.index('Ne_0_epoch_2')])
            N1b = eval(p_list[p_names.index('Ne_1_epoch_2')])
            nus = [N0b / N0, N1b / N0]
