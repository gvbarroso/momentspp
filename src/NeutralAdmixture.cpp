          ...
          if((*it)->getPrefix() == "DD")
          {
                ...
                ++itCmp)
            {
              ...
              if(syntheticDr->isAdmixAdjacent(*itCmp, ancFromId, ancToId))
              {
                ...
                long double x = (0.25 * (*it)->countInstances(ancToId) * c * std::pow((1. - f), a) *
                                 std::pow(f, b)) /
            ...
            if((*it)->getPopIndices()[0] == ancToId) // has pi2 contributions
            {
              ...
              if((*it)->countInstances(ancToId) == 3)
              {
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancFromId, ancFromId, ancFromId));
                coeffs.emplace_back(
                    Eigen::Triplet<long double>(row, col, 4. * std::pow(f, 3.) * (1. - f)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancToId, ancFromId, ancFromId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, 2. * std::pow(f, 2.) * (1. - f) * (1. - 2 * f)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancFromId, ancFromId, ancFromId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, 2. * std::pow(f, 2.) * (1. - f) * (1. - 2 * f)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancFromId, ancToId, ancFromId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, 2. * std::pow(f, 2.) * (1. - f) * (1. - 2 * f)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancFromId, ancFromId, ancToId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, 2. * std::pow(f, 2.) * (1. - f) * (1. - 2 * f)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancFromId, ancToId, ancToId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, -4. * std::pow(f, 2.) * std::pow(1. - f, 2.)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancToId, ancFromId, ancFromId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, -4. * std::pow(f, 2.) * std::pow(1. - f, 2.)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancToId, ancFromId, ancToId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, f * (1. - f) * std::pow(1. - 2. * f, 2.)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancFromId, ancFromId, ancToId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, f * (1. - f) * std::pow(1. - 2. * f, 2.)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancToId, ancToId, ancFromId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, f * (1. - f) * std::pow(1. - 2. * f, 2.)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancFromId, ancToId, ancFromId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, f * (1. - f) * std::pow(1. - 2. * f, 2.)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancToId, ancFromId, ancToId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, -2. * f * std::pow(1. - f, 2.) * (1. - 2. * f)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancToId, ancToId, ancFromId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, -2. * f * std::pow(1. - f, 2.) * (1. - 2. * f)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancToId, ancToId, ancToId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, -2. * f * std::pow(1. - f, 2.) * (1. - 2. * f)));
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancFromId, ancToId, ancToId));
                coeffs.emplace_back(Eigen::Triplet<long double>(
                    row, col, -2. * f * std::pow(1. - f, 2.) * (1. - 2. * f)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancToId, ancToId, ancToId, ancToId));
                coeffs.emplace_back(
                    Eigen::Triplet<long double>(row, col, 4. * f * std::pow(1. - f, 3.)));
              }
                       ...
                       (*it)->getPopIndices()[2] == ancToId))
              {
                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancFromId, ancFromId, ancFromId));
                coeffs.emplace_back(
                    Eigen::Triplet<long double>(row, col, 2. * std::pow(f, 2.) * (1. - f)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancToId, ancFromId, ancFromId));
                coeffs.emplace_back(
                    Eigen::Triplet<long double>(row, col, 2. * f * (1. - f) * (1. - 3. * f)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancFromId, ancToId, ancToId));
                coeffs.emplace_back(
                    Eigen::Triplet<long double>(row, col, -2. * f * std::pow(1. - f, 2.)));

                col = sslib.findCompressedIndex(
                    sslib.findPi2Index(ancFromId, ancToId, ancFromId, ancToId));
                coeffs.emplace_back(
                    Eigen::Triplet<long double>(row, col, -2. * f * (1. - f) * (1. - 2. * f)));
              ...
              else if((*it)->getPopIndices()[1] == ancToId || (*it)->getPopIndices()[2] == ancToId)
              {
                    ...
                    ++itCmp)
                {
                  if(syntheticPi2->isAdmixAdjacent(*itCmp, ancFromId, ancToId))
                  {
                    ...
                    long double z = (tmp->getLeftHetStat()->isCrossPop() &&
                                     tmp->getRightHetStat()->isCrossPop());
                    long double x = 1 + (tmp->getLeftHetStat()->countInstances(ancFromId) == 2 ||
                      ...
                      (*it)->getPopIndices()[2] == ancFromId)
              {
                    ...
                    ++itCmp)
                {
                  if(syntheticPi2->isAdmixAdjacent(*itCmp, ancFromId, ancToId))
                  {
                    ...
                    coeffs.emplace_back(
                        Eigen::Triplet<long double>(row, col, c * 2 * f * (1. - f)));
                  }
                }
              }
              ...
              else
              {
                    ...
                    ++itCmp)
                {
                  if(syntheticPi2->isAdmixAdjacent(*itCmp, ancFromId, ancToId))
                  {
                    ...
                    coeffs.emplace_back(
                        Eigen::Triplet<long double>(row, col, c * d * f * (1. - f)));
                  }
                }
              }
            }
