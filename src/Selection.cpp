  ...
  std::visit(overloaded{[&](auto const& mat)
  {
    ...
    for(size_t i = 0; i < numPops; ++i)
    {
      ...
      for(size_t row = 0; row < basisSize; ++row)
      {
        ...
        size_t popIdCount = moment->countInstances(id); // count of id in moment's name (not counting (1-2p) factors)
        int popIdPower = moment->getPopFactorPower(id); // count of (1-2p_x) factors on focal moment

        std::vector<size_t> popIds = moment->getPopIndices(); // every moment is guaranteed to have popIds.size() > 1
