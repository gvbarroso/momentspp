...
N <- c(10000, 30000, 50000)
u <- 1e-8
r  <- c(1e-4, 1e-5, 1e-6, 1e-7, 1e-8, 1e-9)
s <- c(-1e-5, -2.5e-5, -1e-04, -2.5e-4, -1e-3, -2e-3)
...
for(j in 1:length(orders)) {
  expectation <- numeric()
  for(i in 1:n_models) {
    vals <- read.table(paste("model_",  i, "_", orders[j], 
...
# plots
p1 <- ggplot(data=filter(tbl_pp, stats==commom_stats[1], sample_size==80),
        aes(x=r, y=ratio)) +
  ...
  labs(title="Ratios mpp / m2l, rows->alpha, cols->order", 
       x="r", y=expression(D^2)) +
  geom_hline(yintercept=1, linetype="dashed", color="red") +
  theme(axis.title=element_text(size=12),
        axis.text=element_text(size=10),
        axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
        legend.position="bottom")
save_plot("dd_ratios.png", p1, base_height=24, base_width=12)

p2 <- ggplot(data=filter(tbl_pp, stats==commom_stats[2], sample_size==80),
        aes(x=r, y=ratio)) +
      geom_point(size=3) + geom_line() + theme_bw() + 
  facet_grid(+alpha~order) + scale_x_log10(breaks=r) + 
  ...
  theme(axis.title=element_text(size=12),
        ...
        axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
        legend.position="bottom")
save_plot("dz_ratios.png", p2, base_height=24, base_width=12)

p3 <- ggplot(data=filter(tbl_pp, stats==commom_stats[3], sample_size==80),
        aes(x=r, y=ratio)) +
      geom_point(size=3) + geom_line() + theme_bw() + 
  facet_grid(+alpha~order) + scale_x_log10(breaks=r) + 
  labs(title="Ratios mpp / m2l, rows->alpha, cols->order",
  ...
  theme(axis.title=element_text(size=12),
        ...
        axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
        legend.position="bottom")
save_plot("hl_ratios.png", p3, base_height=24, base_width=12)
...
p4 <- ggplot(data=filter(tbl_pp, stats==commom_stats[4], sample_size==60),
  ...
  theme(axis.title=element_text(size=12),
        ...
        axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
        legend.position="bottom")
save_plot("bench/hr_ratios.png", p4, base_height=24, base_width=12)

p5 <- ggplot(data=filter(tbl_pp, stats==commom_stats[5], sample_size==80),
        aes(x=r, y=ratio)) +
      geom_point(size=3) + geom_line() + theme_bw() +  
  facet_grid(+alpha~order) + scale_x_log10(breaks=r) + 
  labs(title="Ratios mpp / m2l, rows->alpha, cols->order",
  ...
  theme(axis.title=element_text(size=12),
        ...
        axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
        legend.position="bottom")
save_plot("selection/single-pop/bench/pi2_ratios.png",
          p5, base_height=24, base_width=12)
...
for(n in unique(molten_hr$N)) {
  q <- ggplot(data=filter(molten_hr, N==n, r==1e-8), 
    ...
    theme(axis.title=element_text(size=12),
          ...
          legend.position="bottom")
  q
  save_plot(paste("Hr, N=", n, ", r=1e-8.png", sep=""), q)
}

for(a in unique(molten_hr$alpha)) {
 r <- ggplot(data=filter(molten_hr, alpha==a), aes(x=r, y=value, color=method)) +
      geom_point(size=3) + geom_line() + theme_bw() + facet_wrap(~order) +
  scale_x_log10(breaks=r) + scale_y_continuous(breaks=pretty_breaks()) +
  ...
  theme(axis.title=element_text(size=12),
        ...
        legend.position="bottom")
  r
  save_plot(paste("Hr, alpha=", a, ".png", sep=""), r)
}
