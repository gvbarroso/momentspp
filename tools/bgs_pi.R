...
# landscapes. Here we compute B-values for equidistant sampled sites,
...
tbl <- as.data.frame(matrix(ncol=num_inter, nrow=length(B_values))) # viz
...
for(i in 1:num_inter) {
  ...
  getB <- function(focal_exon, focal_samp) { # arguments are indices
