...
knitr::opts_knit$set(root.dir="~/Devel/momentspp/benchmarking/selection/single-pop/")
```

This builds a look-up table on a dense parameter grid that will allow for fast 
assembly of whole chromosomes

```{r, params, include=F, eval=F, results=F, message=F, warning=F}
...
fwrite(dt_r, "landscapes/r.csv", quote=F, row.names=F, col.names=F)
fwrite(dt_s, "landscapes/s.csv", quote=F, row.names=F, col.names=F)
fwrite(params, "landscapes/params.csv", quote=F, row.names=F)

Sys.setenv(num_models=n_models)
```

```{bash dirs, engine.opts='-i', include=F, eval=F, results=F, message=F, warning=F}
...
```{r, demes, include=F, eval=F, results=F, message=F, warning=F}
...
for(i in 1:n_models) {
  ...
  sink(paste("selection/single-pop/landscapes/r_", r, "/", name, 
             ".yaml", sep=""))
...
```{bash moments++, engine.opts='-i', include=F, eval=F, results=F, message=F, warning=F}
...
for(i in 1:length(lookup_r)) {
  for(j in 1:length(lookup_s)) {
    moms <- read.table(paste("landscapes/r_", 
                             lookup_r[i], 
                             "/model_", 
                             as.integer(c), 
...
params$Hr <- hr
fwrite(params, "landscapes/lookup_tbl.csv", row.names=F, quote=F, sep=",")
```

```{r, landscapes, include=F, eval=F, results=F, message=F, warning=F}

lookup_tbl <- fread("landscapes/lookup_tbl.csv")
...
fwrite(models, "test_models/models.csv", sep=",")
```
