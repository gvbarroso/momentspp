...
We want to compare results with SEM
...
```{r plots-lm-1, include=F, message=F, warning=F}
...
Plotting variance explained and (standardized) linear coefficients
...
TODO: plot ratio of PCs (to ratio of CVs)
...
p1 <- ggplot(data=filter(m_pc_sem_p_mutrec, 
      ...
      scale_y_continuous(breaks=pretty_breaks()) +
      scale_x_continuous(breaks=c(0, 432, 864)) + 
      labs(title="Structural Equation Modeling, DAG 2 (mutagenic rec.)",
...
p3 <- ggplot(data=filter(m_pc_sem_p_nonmutrec, 
      ...
      scale_y_continuous(breaks=pretty_breaks()) +
      scale_x_continuous(breaks=c(0, 432, 864)) + 
      labs(title="Structural Equation Modeling, DAG 1 (non-mutagenic rec.)",
    ...
    for(srr in unique(m_pc_sem_p_nonmutrec$shapes_rate_r)) {
      for(mru in unique(m_pc_sem_p_nonmutrec$mult_r_u)) {
        p1 <- ggplot(data=filter(m_pc_sem_p_nonmutrec, 
             ...
             scale_y_continuous(breaks=pretty_breaks()) +
             scale_x_log10() + 
             labs(title="Path coefficients (effects on pi, non-mutagenic DAG)",
                  x="Scale (window size)", y="Path Coefficient",
                  shape="pi", color="Variable") +
             theme(axis.title=element_text(size=16), 
                   axis.text=element_text(size=12), 
...
# mutagenic rec, DAG 2
p1 <- ggplot(data=m_pc_sem_B_mutrec,
            aes(x=model, 
                y=mean_pc, 
                color=as.factor(shapes_rate_u), 
                shape=rhs)) +
...
q1 <- ggplot(data=m_pc_sem_B_nonmutrec,
            aes(x=model, 
                y=mean_pc, 
                color=as.factor(shapes_rate_u), 
                shape=rhs)) +
      ...
      scale_y_continuous(breaks=pretty_breaks()) +
      scale_x_continuous(breaks=c(0, 432, 864)) + 
      labs(title="Structural Equation Modeling, DAG 4 (non-mutagenic rec.)",
           x="Model ID", y="Path Coefficient to B",
           color="Shape Rate Mu", shape="Var") +
      theme(axis.title=element_text(size=16), 
            axis.text=element_text(size=12), 
...
# effect on B independent from source pi, keep simulated only
    ...
    for(srr in unique(m_pc_sem_B$shapes_rate_r)) {
      for(mru in unique(m_pc_sem_B$mult_r_u)) {
        p1 <- ggplot(data=filter(m_pc_sem_B, 
             ...
             scale_y_continuous(breaks=pretty_breaks()) +
             scale_x_log10() + 
             labs(title="Direct effects on B",
                  x="Scale (window size)", y="Path Coefficient",
...
# mutagenic rec, DAG 2
p1 <- ggplot(data=filter(m_pc_sem_recpi, DAG==2),
            aes(x=model, 
                y=mean_pc, 
                color=as.factor(mult_r_u), 
                shape=lhs)) +
...
Building a linear regression that models SEM path coefficients as a function
of parameters shaping chromosome landscapes.

TODO: repeat for rec_x_pi

```{r meta-lm-SEM-pc, include=F, message=F, warning=F}

# path coefficients to pi
