...
for(c in 1:22) {
  dt_1Mb <- filter(human_maps_1Mb, chr==c) %>% 
            dplyr::select(., c(prop_del,
                               avg_mut,
                               avg_rec,
                               B,
                               exp_pi,
  ...
  p <- ggpairs(dt_1Mb)
  save_plot(paste("Correlations/pairs_chr", c, ".png", sep=""), p,
...
num_chrom <- 22
num_reps <- 1e+3 # sampling the dataframe for reducing autocorrelation
...
for(c in 1:num_chrom) {
  setTxtProgressBar(pb, c)
  df <- filter(human_maps_1Mb, chr==c, num_sites > 5e+5, !is.na(avg_pi)) 
  for(r in 1:num_reps) {

    samp <- sample_n(df, nrow(df), replace=T)
    pvals_1Mb <- rep(NA, length(basis_set))
    ...
    dsep_c_1Mb[r, c] <- combine.test(pvals_1Mb, weights, "fisher", na.rm=T)
  }
}
close(pb)
...
# NOTE: using Kendall's (tau-b) correlation to handle ties
pb <- txtProgressBar(min=0, max=num_chrom, style=3)  
for(c in 1:num_chrom) {
  setTxtProgressBar(pb, c)
  df <- filter(human_maps_1Mb, chr==c, num_sites > 5e+5, !is.na(avg_pi)) 
...
fwrite(dsep_c_1Mb, "real_data_maps/dsep_1Mb_pisim_m5.csv.gz")
```
...
# combines tables from previous chunks.
pval_dsep1 <- fread("real_data_maps/dsep_1Mb_pisim_m1.csv.gz") 
pval_dsep2 <- fread("real_data_maps/dsep_1Mb_pisim_m2.csv.gz") 
pval_dsep3 <- fread("real_data_maps/dsep_1Mb_pisim_m3.csv.gz") 
pval_dsep4 <- fread("real_data_maps/dsep_1Mb_pisim_m4.csv.gz") 
pval_dsep5 <- fread("real_data_maps/dsep_1Mb_pisim_m5.csv.gz") 
...
pval_dsep1$boot <- 1:nrow(pval_dsep1)
pval_dsep2$boot <- 1:nrow(pval_dsep2)
pval_dsep3$boot <- 1:nrow(pval_dsep3)
pval_dsep4$boot <- 1:nrow(pval_dsep4)
pval_dsep5$boot <- 1:nrow(pval_dsep5)

dsep_human_maps_1Mb <- rbind.data.frame(pval_dsep1,
                                        pval_dsep2,
                                        pval_dsep3,
                                        pval_dsep4,
                                        pval_dsep5)
names(dsep_human_maps_1Mb) <- c(as.character(1:22), "DAG", "boot")
fwrite(dsep_human_maps_1Mb, "real_data_maps/dsep_human_maps_1Mb_pisim.csv.gz")
```
...
fwrite(dsep_c_1Mb, "real_data_maps/dsep_1Mb_piobs_m1.csv.gz")
```
...
fwrite(dsep_c_1Mb, "real_data_maps/dsep_1Mb_piobs_m2.csv.gz")
```
...
fwrite(dsep_c_1Mb, "real_data_maps/dsep_1Mb_piobs_m3.csv.gz")
```
...
fwrite(dsep_c_1Mb, "real_data_maps/dsep_1Mb_piobs_m4.csv.gz")
```
...
D5 <- DAG(
  ...
  B~prop_del_sites,
  avg_mut~avg_rec,
  # 2nd order selection on the mutation and recombination landscapes?
  avg_mut~prop_del_sites,
  avg_rec~prop_del_sites
)
...
fwrite(dsep_c_1Mb, "real_data_maps/dsep_1Mb_piobs_m5.csv.gz")
```

Writing to file 

```{r d-sep-plot-piobs, include=F, message=F, warning=F}

# combines tables from previous chunks.
pval_dsep1 <- fread("real_data_maps/dsep_1Mb_piobs_m1.csv.gz") 
pval_dsep2 <- fread("real_data_maps/dsep_1Mb_piobs_m2.csv.gz") 
pval_dsep3 <- fread("real_data_maps/dsep_1Mb_piobs_m3.csv.gz") 
pval_dsep4 <- fread("real_data_maps/dsep_1Mb_piobs_m4.csv.gz") 
pval_dsep5 <- fread("real_data_maps/dsep_1Mb_piobs_m5.csv.gz") 
...
dsep_human_maps_1Mb <- rbind.data.frame(pval_dsep1,
                                        ...
                                        pval_dsep5)
names(dsep_human_maps_1Mb) <- c(as.character(1:22), "DAG", "boot")
fwrite(dsep_human_maps_1Mb, "real_data_maps/dsep_human_maps_1Mb_piobs.csv.gz")
```
