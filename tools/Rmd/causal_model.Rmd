...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    mat=Hmisc::rcorr(as.matrix(maps_1kb), type="spearman")
    tbl_1kb <- mat$r
    ...
    std_1kb <- as.data.frame(apply(maps_1kb, 2, function(x) (x-mean(x)) / sd(x)))
    std_1kb$bin <- 1:nrow(std_1kb)
    
    std_10kb <- as.data.frame(apply(maps_10kb, 2, function(x) (x-mean(x)) / sd(x)))
    std_10kb$bin <- 1:nrow(std_10kb)
    
    std_100kb <- as.data.frame(apply(maps_100kb, 2, function(x) (x-mean(x)) / sd(x)))
    std_100kb$bin <- 1:nrow(std_100kb)
    
    std_1Mb <- as.data.frame(apply(maps_1Mb, 2, function(x) (x-mean(x)) / sd(x)))
    std_1Mb$bin <- 1:nrow(std_1Mb)
    
    # 1 kb
    m_1kb <- lm(sim_pi ~ (B + avg_mut) ^ 2, std_1kb)
    summary(m_1kb)
    ...
    # 10 kb
    m_10kb <- lm(sim_pi ~ (B + avg_mut) ^ 2, std_10kb)
    summary(m_10kb)
