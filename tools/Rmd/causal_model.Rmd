...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    corr_tbl <- as.data.frame(matrix(ncol=3, nrow=3))
    ...
    corr_tbl$Predicted[1] <- cor.test(maps_10kb$sim_pi, 
                                      maps_10kb$pi_exp, 
                                      method="pearson")$estimate 
    corr_tbl$Predicted[2] <- cor.test(maps_100kb$sim_pi, 
                                      maps_100kb$pi_exp,
                                      method="pearson")$estimate  
    corr_tbl$Predicted[3] <- cor.test(maps_1Mb$sim_pi,
                                      maps_1Mb$pi_exp, 
                                      method="pearson")$estimate 
...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    row <- which(names(cors_10kb) == "pi_exp")
...
r_vs_piexp <- rbind.data.frame(r_vs_piexp_10kb,
                               r_vs_piexp_100kb,
                               r_vs_piexp_1Mb)
...
u_vs_piexp <- rbind.data.frame(u_vs_piexp_10kb,
                               u_vs_piexp_100kb,
                               u_vs_piexp_1Mb)
...
B_vs_piexp <- rbind.data.frame(B_vs_piexp_10kb,
                               B_vs_piexp_100kb,
                               B_vs_piexp_1Mb)
...
for(i in 1:num_models) {
  ...
  for(j in 1:num_reps) {
    ...
    r2_raw_10kb[j, 1] <- tmp[1, 1]
    r2_raw_100kb[j, 1] <- tmp[2, 1]
    r2_raw_1Mb[j, 1] <- tmp[3, 1]
  }
  ...
  for(j in 1:num_reps) {
    ...
    r2_raw_10kb[j, 2] <- tmp[1, 1]
    r2_raw_100kb[j, 2] <- tmp[2, 1]
    r2_raw_1Mb[j, 2] <- tmp[3, 1]
  }
...
NOTE: del_sites, a weighted verion of exonic_sites, is computed later on
...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    # gets sum of relevant deleterious elements, weighted by inverse rec rate
    ## 10 kb
    ...
    for(d in 1:nrow(prd)) {
      prd[d, d] <- maps_10kb$avg_rec[d] * 1e+4 # filling in diagonal
    }
    ...
    relevant_exons <- apply(prd, 2, function(x) x < 5e-2)
    exons_per_bin <- apply(relevant_exons, 1, function(x) which(x))
    ...
    ## filter bins to avoid auto-correlation
    ...
    std_10kb <- as.data.frame(apply(apply(dplyr::select(filt_10kb,
      -c(exonic_sites, cum_rec, pi_del_exp, pi_neu_exp, pi_neu_sim)), 2, log),
    ...
    filt_100kb <- filter(std_1Mb, del_sites > 0) 
...
# Simulated data with Human maps
...
Without interference, this follows the lavaan DAGs with pi expected, therefore,
we can treat the maps as "true" and avoid modeling error in their estimation.

```{r human-maps-piexp, include=F, message=F, warning=F}

human_maps_1kb <- data.frame()

pb <- txtProgressBar(min=0, max=22, style=3)
for(c in 1:22) {
  setTxtProgressBar(pb, c)
  tbl_c <- fread(paste("real_data_maps/equilibrium_human_predictions/",
                       "functional/no_interf_correction/",
                       "tbl_YRI_functional_chr", c, "_1kb.csv.gz", sep=""))
  tbl_c$`#chrom` <- c
  tbl_c$bin_1kb <- 1:nrow(tbl_c)
  tbl_c$bin_10kb <- (tbl_c$bin_1kb - 1) %/% 10
  tbl_c$bin_100kb <- (tbl_c$bin_1kb - 1) %/% 100
  tbl_c$bin_1Mb <- (tbl_c$bin_1kb - 1) %/% 1000
    
  human_maps_1kb <- rbind.data.frame(human_maps_1kb, tbl_c)
}
close(pb)

names(human_maps_1kb)[1] <- "chr" # facilitate calling

# 10 kb
human_maps_10kb <- human_maps_1kb %>% group_by(chr, bin_10kb) %>% 
                   summarise_at(c("avg_mut", 
                                  "avg_rec", 
                                  "B", 
                                  "neu_pi0",
                                  "exp_pi",
                                  ...
                                  "num_sites",
                                  "del_sites"), mean)
# for number of sites we want their sum in each bin
human_maps_10kb$num_sites <- human_maps_10kb$num_sites * 10
human_maps_10kb$del_sites <- human_maps_10kb$del_sites * 10
human_filtered_10kb <- dplyr::select(human_maps_10kb, -bin_10kb) %>%
                       filter(., num_sites > 7500)

# 100 kb
human_maps_100kb <- human_maps_1kb %>% group_by(chr, bin_100kb) %>% 
                    summarise_at(c("avg_mut", 
                                   "avg_rec", 
                                   "B", 
                                   "neu_pi0",
                                   ...
                                   "del_sites"), mean)
# for number of sites we want their sum in each bin
human_maps_100kb$num_sites <- human_maps_100kb$num_sites * 100
human_maps_100kb$del_sites <- human_maps_100kb$del_sites * 100
human_filtered_100kb <- dplyr::select(human_maps_100kb, -bin_100kb) %>%
                        filter(., num_sites > 75000)

# 1 Mb
human_maps_1Mb <- human_maps_1kb %>% group_by(chr, bin_1Mb) %>% 
                  summarise_at(c("avg_mut", 
                                 "avg_rec", 
                                 "B", 
                                 "neu_pi0",
...
# for number of sites we want their sum in each bin
human_maps_1Mb$num_sites <- human_maps_1Mb$num_sites * 1000
human_maps_1Mb$del_sites <- human_maps_1Mb$del_sites * 1000
human_filtered_1Mb <- dplyr::select(human_maps_1Mb, -bin_1Mb) %>%
                        filter(., num_sites > 750000)

# summarizing in terms of AC and CV
human_summaries_10kb <- human_filtered_10kb %>% group_by(chr) %>%
...
human_summaries_100kb <- human_filtered_100kb %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
human_summaries_1Mb <- human_filtered_1Mb %>% group_by(chr) %>%
                       mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                       mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                       mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                           lag=1, 
...
d-Sep in humans
NOTE auto-correlation is a problem at large scales as well

```{r d-sep-DAG1-humanmaps, include=F, message=F, warning=F}

basis_set <- bs1
weights <- rep(1, length=length(basis_set))

dsep_dag1_human_maps <- as.data.frame(matrix(nrow=3, ncol=22))

# match column names with DAG labels
# NOTE testing
names(human_filtered_10kb)[which(names(human_filtered_10kb)=="exp_pi")] <- "pi_neu_exp"
names(human_filtered_100kb)[which(names(human_filtered_100kb)=="exp_pi")] <- "pi_neu_exp"
names(human_filtered_1Mb)[which(names(human_filtered_1Mb)=="exp_pi")] <- "pi_neu_exp"

names(human_filtered_10kb)[which(names(human_filtered_10kb)=="del_sites")] <- "exonic_sites"
names(human_filtered_100kb)[which(names(human_filtered_100kb)=="del_sites")] <- "exonic_sites"
names(human_filtered_1Mb)[which(names(human_filtered_1Mb)=="del_sites")] <- "exonic_sites"

for(c in 1:22) {
  
  human_c_10kb <- filter(human_filtered_10kb, chr==c) %>% as.data.table()
  human_c_100kb <- filter(human_filtered_100kb, chr==c) %>% as.data.table()
  human_c_1Mb <- filter(human_filtered_1Mb, chr==c) %>% as.data.table()
  ...
  for(i in 1:length(basis_set)) {
    ...
    if(length(basis_set[[i]]) > 2) {
      tryCatch({
        pvals_10kb[i] <- ppcor::pcor.test(human_c_10kb[, ..var1], 
                                          human_c_10kb[, ..var2], 
                                          human_c_10kb[, ..cond],
                                          method="spearman")$p.value
      }, error=function(e){cat("WARNING :",conditionMessage(e), "\n")})
      tryCatch({
        pvals_100kb[i] <- ppcor::pcor.test(human_c_100kb[, ..var1], 
                                           human_c_100kb[, ..var2], 
                                           human_c_100kb[, ..cond],
                                           method="spearman")$p.value
      }, error=function(e){cat("WARNING :",conditionMessage(e), "\n")})
      tryCatch({
        pvals_1Mb[i] <- ppcor::pcor.test(human_c_1Mb[, ..var1], 
                                         human_c_1Mb[, ..var2], 
                                         human_c_1Mb[, ..cond],
                                         method="spearman")$p.value
      }, error=function(e){cat("WARNING :",conditionMessage(e), "\n")})
    } else {
      pvals_10kb[i] <- cor.test(human_c_10kb[, ..var1][[1]],
                                human_c_10kb[, ..var2][[1]],
                                method="spearman")$p.value
      
      pvals_100kb[i] <- cor.test(human_c_100kb[, ..var1][[1]],
                                 human_c_100kb[, ..var2][[1]],
                                 method="spearman")$p.value
      
      pvals_1Mb[i] <- cor.test(human_c_1Mb[, ..var1][[1]],
                               human_c_1Mb[, ..var2][[1]],
                               method="spearman")$p.value
    }
  }
  
  dsep_dag1_human_maps[1, c] <- combine.test(pvals_10kb, 
                                             weights, 
                                             "fisher", 
                                             na.rm=T)
  dsep_dag1_human_maps[2, c] <- combine.test(pvals_100kb, 
                                             weights, 
                                             "fisher",
                                             na.rm=T)
  dsep_dag1_human_maps[3, c] <- combine.test(pvals_1Mb, 
                                             weights, 
                                             "fisher",
                                             na.rm=T)
}
