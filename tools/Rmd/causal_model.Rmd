...
maps_10kb$mean_mut_roulette[which(maps_10kb$sum_sites_roulette < 7.5e+3)] <- NA
maps_10kb$mean_mut_masked_roulette[which(maps_10kb$sum_sites_masked_roulette < 7.5e+3)] <- NA
maps_10kb$mean_mut_carlson[which(maps_10kb$sum_sites_carlson < 5.5e+3)] <- NA
maps_10kb$mean_mut_masked_carlson[which(maps_10kb$sum_sites_masked_carlson < 7.5e+3)] <- NA
maps_10kb$mean_mut_gnomad[which(maps_10kb$sum_sites_gnomad < 7.5e+3)] <- NA
maps_10kb$mean_mut_masked_gnomad[which(maps_10kb$sum_sites_masked_gnomad < 7.5e+3)] <- NA

maps_100kb$mean_mut_roulette[which(maps_100kb$sum_sites_roulette < 7.5e+4)] <- NA
maps_100kb$mean_mut_masked_roulette[which(maps_100kb$sum_sites_masked_roulette < 7.5e+4)] <- NA
maps_100kb$mean_mut_carlson[which(maps_100kb$sum_sites_carlson < 7.5e+4)] <- NA
maps_100kb$mean_mut_masked_carlson[which(maps_100kb$sum_sites_masked_carlson < 7.5e+4)] <- NA
maps_100kb$mean_mut_gnomad[which(maps_100kb$sum_sites_gnomad < 7.5e+4)] <- NA
maps_100kb$mean_mut_masked_gnomad[which(maps_100kb$sum_sites_masked_gnomad < 7.5e+4)] <- NA

maps_1Mb$mean_mut_roulette[which(maps_1Mb$sum_sites_roulette < 7.5e+5)] <- NA
maps_1Mb$mean_mut_masked_roulette[which(maps_1Mb$sum_sites_masked_roulette < 7.5e+5)] <- NA
maps_1Mb$mean_mut_carlson[which(maps_1Mb$sum_sites_carlson < 7.5e+5)] <- NA
maps_1Mb$mean_mut_masked_carlson[which(maps_1Mb$sum_sites_masked_carlson < 7.5e+5)] <- NA
maps_1Mb$mean_mut_gnomad[which(maps_1Mb$sum_sites_gnomad < 7.5e+5)] <- NA
maps_1Mb$mean_mut_masked_gnomad[which(maps_1Mb$sum_sites_masked_gnomad < 7.5e+5)] <- NA
...
tbl_chr1_1Mb <-  filter(df_chr1_1Mb, coverage > 0.5)

p1 <- ggpairs(dplyr::select(tbl_chr1_1Mb, c(mean_mut_masked_carlson,
                                            mean_mut_masked_roulette,
                                            mean_mut_masked_gnomad,
                                            ismc_1,
                                            ismc_2,
...
tbl_chr1_1Mb_log <- cbind.data.frame(tbl_chr1_1Mb[,1:3],
                                     apply(tbl_chr1_1Mb[,4:ncol(tbl_chr1_1Mb)-1],
                                           2, log))
tbl_chr1_1Mb_log$ismc_latent <- tbl_chr1_1Mb$ismc_latent
...
mut_measurement_2 <- "
mu=~NA*mean_mut_masked_roulette + mean_mut_masked_carlson + mean_mut_masked_gnomad + ismc_mean 
...
maps_1kb <- SparseSummarizedExperiment::combine(pyrho_CEU_gr, 
                                                pyrho_YRI_gr,
                                                decode_gr)

maps_1kb <- as.data.table(maps_1kb) %>% dplyr::select(., c(seqnames,
                                                           start, 
                                                           end, 
                                                           pyrho_CEU,
                                                           pyrho_YRI,
                                                           deCODE))
names(maps_1kb)[1:3] <- c("chrom", "chromStart", "chromEnd")
...
df_chr1_1Mb <- left_join(filter(maps_1Mb, chrom=="chr1"),
                         ismc_chr1,
                         by=c("chrom", "chromStart", "chromEnd"))
...
tbl_chr1_1Mb_log <- cbind.data.frame(df_chr1_1Mb[,1:3],
                                     apply(df_chr1_1Mb[,4:ncol(df_chr1_1Mb)-1],
                                           2, log))
tbl_chr1_1Mb_log$ismc_latent <- df_chr1_1Mb$ismc_latent
...
rec_measurement_2 <- "
rec=~mean_pyrho_CEU + mean_pyrho_YRI + mean_deCODE + ismc_latent 
mean_pyrho_CEU ~~ ismc_latent
" 

m4 <- sem(model=rec_measurement_2,
          data=tbl_chr1_1Mb_log,
          std.ov=T,
          std.lv=T,
          missing="fiml")
...
Putting it together (chr 1, 1Mb scale)

```{r real-data, include=F, message=F, warning=F}

# adjusting rec tbl
rec_chr1_1Mb <- filter(maps_1Mb, chrom=="chr1")
rec_chr1_1Mb$chrom <- 1
rec_chr1_1Mb$chrom <- as.integer(rec_chr1_1Mb$chrom)

df_chr1_1Mb <- left_join(tbl_chr1_1Mb, rec_chr1_1Mb,
                         by=c("chrom", "chromStart", "chromEnd"))
...
# regressions
avg_pi ~ start(0.25)*c1*B
avg_pi ~ start(0.25)*c2*mu
B ~ start(-0.25)*c3*mean_mut_masked_roulette
B ~ start(-0.25)*c4*prop_del_sites
B ~ start(-0.25)*c5*mean_pyrho_YRI
"

m0gls <- sem(model_0,
             data=na.omit(tbl_chr1_1Mb_log),
             fixed.x=F,
             std.ov=T,
             std.lv=T,
             estimator="GLS")
...
B ~ start(-0.25)*c4*prop_del_sites
B ~ start(-0.25)*c5*mean_pyrho_YRI
mean_pyrho_YRI~~avg_pi
"


TODO try using the latent mu directly to reduce % missing data (imputation?)
m1gls <- sem(model_1,
             data=na.omit(tbl_chr1_1Mb_log),
             fixed.x=F,
             std.ov=T,
