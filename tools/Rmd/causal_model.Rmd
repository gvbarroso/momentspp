...
Preparing the data (Functional model with split exons)
...
human_maps_10kb_roulette <- filter(human_maps_10kb_roulette, coverage > 0.75)
human_maps_10kb_carlson <- filter(human_maps_10kb_carlson, coverage > 0.75)
human_maps_10kb_gnomad <- filter(human_maps_10kb_gnomad, coverage > 0.75)
human_maps_10kb_uniform <- filter(human_maps_10kb_uniform, coverage > 0.75)

human_maps_10kb_roulette <- human_maps_10kb_roulette %>% group_by(chr) %>%
                            mutate(bin_100kb=chromEnd %/% 1e+5,
                                   bin_1Mb=chromEnd %/% 1e+6)
human_maps_10kb_carlson <- human_maps_10kb_carlson %>% group_by(chr) %>%
                           mutate(bin_100kb=chromEnd %/% 1e+5,
                                  bin_1Mb=chromEnd %/% 1e+6)
human_maps_10kb_gnomad <- human_maps_10kb_gnomad %>% group_by(chr) %>%
                           mutate(bin_100kb=chromEnd %/% 1e+5,
                                  bin_1Mb=chromEnd %/% 1e+6)
human_maps_10kb_uniform <- human_maps_10kb_uniform %>% group_by(chr) %>%
                           mutate(bin_100kb=chromEnd %/% 1e+5,
                                  bin_1Mb=chromEnd %/% 1e+6)

human_maps_10kb_roulette$mut_map <- "roulette"
human_maps_10kb_carlson$mut_map <- "roulette"
...
## 10 kb
human_summaries_10kb_roulette <- human_maps_10kb_roulette %>% group_by(chr) %>%
...
human_summaries_10kb_carlson <- human_maps_10kb_carlson %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
human_summaries_10kb_gnomad <- human_maps_10kb_gnomad %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
## 100 kb
human_summaries_100kb_roulette <- human_maps_100kb_roulette %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
human_summaries_100kb_carlson <- human_maps_100kb_carlson %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
human_summaries_100kb_gnomad <- human_maps_100kb_gnomad %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
## 1 Mb
human_summaries_1Mb_roulette <- human_maps_1Mb_roulette %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
human_summaries_1Mb_carlson <- human_maps_1Mb_carlson %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
human_summaries_1Mb_gnomad <- human_maps_1Mb_gnomad %>% group_by(chr) %>%
                            mutate(cv_mut=sd(avg_mut)/mean(avg_mut)) %>%
                            mutate(cv_rec=sd(avg_rec)/mean(avg_rec)) %>%
                            mutate(ac_mut=as.numeric(unlist(acf(avg_mut,
                                                                lag=1, 
...
# easier viz
human_summaries_roulette$chr <- paste("chr", human_summaries_roulette$chr, sep="") 
human_summaries_carlson$chr <- paste("chr", human_summaries_carlson$chr, sep="") 
human_summaries_gnomad$chr <- paste("chr", human_summaries_gnomad$chr, sep="") 
...
for(c in c(1:18, 20:22)) { # chr19 doesn't have enough data w/ 75% threshold
  dt_1Mb <- human_maps_1Mb[human_maps_1Mb$chrom==c & 
                           human_maps_1Mb$num_sites>5e+5,] %>% 
  #dt_1Mb <- dplyr::filter(human_maps_1Mb, chrom==c, num_sites > 5e+5) %>% 
  ...
  p <- ggpairs(dt_1Mb)
  save_plot(paste("Correlations/pairs_1Mb_chr", c, ".png", sep=""), p,
            base_height=10, base_width=12)
}

for(c in c(1:18, 20:22)) {
  dt_100kb <- human_maps_100kb[human_maps_100kb$chrom==c & 
                               human_maps_100kb$num_sites>7.5e+4,] %>% 
  #dt_100kb <- filter(human_maps_100kb, chrom==c, num_sites > 7.5e+4) %>% 
            dplyr::select(., c(prop_del_sites,
                               avg_mut,
                               avg_rec,
                               B,
  ...
  p <- ggpairs(dt_100kb)
  save_plot(paste("Correlations/pairs_100kb_chr", c, ".png", sep=""), p,
            base_height=10, base_width=12)
}

for(c in c(1:18, 20:22)) {
  dt_10kb <- human_maps_10kb[human_maps_10kb$chrom==c & 
                             human_maps_10kb$num_sites>7.5e+3,] %>% 
  #dt_10kb <- filter(human_maps_10kb, chrom==c, num_sites > 7.5e+3) %>% 
             dplyr::select(., c(prop_del_sites,
                                avg_mut,
                                avg_rec,
                                B,
  ...
  p <- ggpairs(dt_10kb)
  save_plot(paste("Correlations/pairs_10kb_chr", c, ".png", sep=""), p,
            base_height=10, base_width=12)
}
```
...
for(u in unique(human_maps_1Mb$mut_map)) {
  ...
  dat_1Mb <- human_maps_1Mb %>% 
             ...
             filter(., mut_map == u,
                    del_sites > 0,
                    num_sites > 5e+5,
                    chr!=19) %>%  # chr 19 too few data points
             na.exclude() 
  ...
  fwrite(parameterestimates(m1gls, ci=F), paste("SEM/m1gls_est_",
                                                u, ".csv", sep=""))
  fwrite(t(as.data.frame(fitMeasures(m1gls))), paste("SEM/m1gls_fit_",
                                                     u, ".csv", sep=""))
  fwrite(as.data.frame(inspect(m1gls, "rsquare")), paste("SEM/m1gls_r2_",
                                                   u, ".csv", sep="")) 
  ...
  fwrite(parameterestimates(m2gls, ci=F), paste("SEM/m2gls_est_", 
                                                u, ".csv", sep=""))
  fwrite(t(as.data.frame(fitMeasures(m2gls))), paste("SEM/m2gls_fit_",
                                                     u, ".csv", sep=""))
  fwrite(as.data.frame(inspect(m2gls, "rsquare")), paste("SEM/m2gls_r2_",
                                                   u, ".csv", sep="")) 
  ...
  avg_pi ~ start(-0.25)*c3*prop_del_sites
  B ~ start(-0.25)*c4*avg_mut
  B ~ start(0.1)*c5*avg_rec
  avg_mut ~ start(0.1)*c6*avg_rec
  "
  ...
  fwrite(parameterestimates(m3gls, ci=F), paste("SEM/m3gls_est_", 
                                                u, ".csv", sep=""))
  fwrite(t(as.data.frame(fitMeasures(m3gls))), paste("SEM/m3gls_fit_",
                                                     u, ".csv", sep=""))
  fwrite(as.data.frame(inspect(m3gls, "rsquare")), paste("SEM/m3gls_r2_",
                                                   u, ".csv", sep=""))  
  ...
  fwrite(parameterestimates(m4gls, ci=F), paste("SEM/m4gls_est_", 
                                                u, ".csv", sep=""))
  fwrite(t(as.data.frame(fitMeasures(m4gls))), paste("SEM/m4gls_fit_",
                                                     u, ".csv", sep=""))
  fwrite(as.data.frame(inspect(m4gls, "rsquare")), paste("SEM/m4gls_r2_",
                                                   u, ".csv", sep="")) 
  ...
  fwrite(parameterestimates(m5gls, ci=F), paste("SEM/m5gls_est_", 
                                                u, ".csv", sep=""))
  fwrite(t(as.data.frame(fitMeasures(m5gls))), paste("SEM/m5gls_fit_",
                                                     u, ".csv", sep=""))
  fwrite(as.data.frame(inspect(m5gls, "rsquare")), paste("SEM/m5gls_r2_",
                                                   u, ".csv", sep=""))  
  ...
  fwrite(parameterestimates(m6gls, ci=F), paste("SEM/m6gls_est_", 
                                                u, ".csv", sep=""))
  fwrite(t(as.data.frame(fitMeasures(m6gls))), paste("SEM/m6gls_fit_",
                                                     u, ".csv", sep=""))
  fwrite(as.data.frame(inspect(m6gls, "rsquare")), paste("SEM/m6gls_r2_",
                                                   u, ".csv", sep="")) 
  ...
  B ~ start(0.25)*c10*avg_pi
  avg_mut ~ start(0.25)*c7*avg_rec
  prop_del_sites ~ start(0.25)*c8*avg_mut
  prop_del_sites ~ start(0.25)*c9*avg_rec
  avg_rec ~ start(0.25)*c10*prop_del_sites
  avg_mut ~~ B
  "
  ...
  fwrite(parameterestimates(m7gls, ci=F), paste("SEM/m7gls_est_", 
                                                u, ".csv", sep=""))
  fwrite(t(as.data.frame(fitMeasures(m7gls))), paste("SEM/m7gls_fit_",
                                                     u, ".csv", sep=""))
  fwrite(as.data.frame(inspect(m7gls, "rsquare")), paste("SEM/m7gls_r2_",
                                                   u, ".csv", sep="")) 
}
```
