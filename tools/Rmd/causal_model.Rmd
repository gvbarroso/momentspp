    ...
    for(srr in unique(m_pc_sem_recpi$shapes_rate_r)) {
      for(mru in unique(m_pc_sem_recpi$mult_r_u)) {
        p1 <- ggplot(data=filter(m_pc_sem_recpi, 
             ...
             scale_y_continuous(breaks=pretty_breaks()) +
             scale_x_log10() + 
             labs(title="Effects of recombination on pi (finding the Przeworski regime)",
                  x="Scale (window size)", y="Path Coefficient",
...
for(i in 1:num_models) {
  ...
  # in expectation
  for(j in 1:num_reps) {
    tmp <- fread(paste("model_", i, "/rep_", j, "/r2_typeII_tbl.csv", sep=""))
    ...
    tmp <- fread(paste("model_", i, "/rep_", j, "/coeff_tbl.csv", sep=""))
...
TODO: plot simple correlations, build lm on path coefficients
TODO: continue here
