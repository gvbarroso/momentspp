...
p1 <- ggplot(data=filter(m_pval_sem, DAG==1),
...
p2 <- ggplot(data=filter(m_pval_sem, DAG==2),
             aes(x=model, y=-log(pval), 
             shape=as.factor(100* exon_lengths * num_exons / L), 
             color=as.factor(mult_r_u))) +
      geom_hline(aes(yintercept=-log10(0.05/num_reps)), 
...
p3 <- ggplot(data=filter(m_pval_sem, DAG==3),
             aes(x=model, y=-log(pval), 
             shape=as.factor(100* exon_lengths * num_exons / L), 
             color=as.factor(mult_r_u))) +
      geom_hline(aes(yintercept=-log10(0.05/num_reps)), 
...
p4 <- ggplot(data=filter(m_pval_sem, DAG==4),
             aes(x=model, y=-log(pval), 
             shape=as.factor(100* exon_lengths * num_exons / L), 
             color=as.factor(mult_r_u))) +
      geom_hline(aes(yintercept=-log10(0.05/num_reps)), 
...
Path Coefficient plots 
TODO: CHECK FOR MODEL CONVERGENCE (NA'S IN P-VALUE TABLE)

```{r path_coeff-plots-SEM, include=F, message=F, warning=F}
...
pc_nonmut_rec_pi <- filter(pc_nonmut_rec, lhs=="pi_exp" | lhs=="sim_pi")
pc_mut_rec_pi <- filter(pc_mut_rec, lhs=="pi_exp" | lhs=="sim_pi")
...
# adding "indicator" variable for source of pi
m_pc_sem_p_mutrec$pi_source <- ifelse(m_pc_sem_p_mutrec$DAG == 2, "exp", "sim")
m_pc_sem_p_nonmutrec$pi_source <- ifelse(m_pc_sem_p_nonmutrec$DAG == 1, "exp", "sim")
    ...
    for(srr in unique(m_pc_sem_p_mutrec$shapes_rate_r)) {
      for(mru in unique(m_pc_sem_p_mutrec$mult_r_u)) {
        p1 <- ggplot(data=filter(m_pc_sem_p_mutrec, 
             ...
             scale_y_continuous(breaks=pretty_breaks()) +
             scale_x_log10() + 
             labs(title="Direct effects on pi",
        ...
        fout <- paste("pcoeff_mutagenic_DAG_exons", e, 
                      "_ars", ars, 
                      "_srr", srr, 
                      "_mru", mru,
                      ".png", sep="")
  ...
  for(ars in unique(m_pc_sem_p_nonmutrec$avg_rec_spans)) {
    for(srr in unique(m_pc_sem_p_nonmutrec$shapes_rate_r)) {
      for(mru in unique(m_pc_sem_p_nonmutrec$mult_r_u)) {
        ...
        fout <- paste("pcoeff_non-mutagenic_DAG_exons", e, 
                       "_ars", ars, 
                       "_srr", srr, 
                       "_mru", mru,
                       ".png", sep="")
...
TODO: plot PCs other than direct effects
```
...
for(i in 1:num_models) {
  ...
  for(j in 1:num_reps) {
    ...
    m1Mb$end <- seq(1e+6, by=1e+6, length.out=nrow(m1Mb)) 
