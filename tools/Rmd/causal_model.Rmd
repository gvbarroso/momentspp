...
p_switch <- 0.25 # related to auto-correlation in the distribution of elements
# note since average length of intergenic > exon_lengths, p_switch > 0 => AR > 0

models <- setDT(crossing(L, p_switch, num_exons, exon_lengths, mult_r_u,
...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    p_switch <- models$p_switch[m] # symmetric auto-correlation
    ...
    for(i in 2:length(new_elements)) {
      ...
      if(runif(1, 0, 1) > p_switch) { # switch from/to exonic/intergenic
        is_exonic <- !is_exonic
      }
    }
...
```{r d-sep, include=F, message=F, warning=F}

tbl <- fread("model_1/rep_1/tbl_1kb.csv.gz") # testing
basis_set <- bs1
...
for(i in 1:length(basis_set)) {
  ...
  if(length(basis_set[[i]]) > 2) {
    print(ppcor::pcor.test(tbl[, ..var1], tbl[, ..var2], tbl[, ..cond],
                           method="spearman"))
  } else {
    print(cor.test(tbl[, ..var1][[1]], tbl[, ..var2][[1]], method="spearman"))
  }
}
```

The good old linear models (but not only)
...
for(m in 1:num_models) {
  ...
  for(r in 1:num_reps) {
    ...
    maps_1kb <- fread(paste("model_", m, "/rep_", r, "/tbl_1kb.csv.gz", sep=""))
    ...
    t1kb=dplyr::select(maps_1kb, c(avg_rec,
                                   avg_mut,
                                   exonic_sites,
                                   B,
                                   pi_neu_sim))
    mat=Hmisc::rcorr(as.matrix(t1kb), type="spearman")
    tbl_1kb <- mat$r
    tbl_1kb[upper.tri(tbl_1kb)] <- mat$P[upper.tri(mat$P)]
    suppressMessages(fwrite(tbl_1kb, paste("model_", m, "/rep_", r, 
                                           "/pval_upper_spearman_lower_1kb.csv", 
                                           sep="")))
    
    t10kb=dplyr::select(maps_10kb, c(avg_rec,
                                     avg_mut,
                                     exonic_sites,
                                     B,
                                     pi_neu_sim))
    mat=Hmisc::rcorr(as.matrix(t10kb), type="spearman")
    tbl_10kb <- mat$r
    tbl[upper.tri(tbl_10kb)] <- mat$P[upper.tri(mat$P)]
    suppressMessages(fwrite(tbl_10kb, paste("model_", m, "/rep_", r, 
                                            "/pval_upper_spearman_lower_10kb.csv", 
                                            sep="")))
    
    t100kb=dplyr::select(maps_100kb, c(avg_rec,
                                       avg_mut,
                                       exonic_sites,
                                       B,
                                       pi_neu_sim))
    mat=Hmisc::rcorr(as.matrix(t100kb), type="spearman")
    tbl_100kb <- mat$r
    tbl_100kb[upper.tri(tbl_100kb)] <- mat$P[upper.tri(mat$P)]
    suppressMessages(fwrite(tbl_100kb, paste("model_", m, "/rep_", r, 
                                             "/pval_upper_spearman_lower_100kb.csv", 
                                             sep="")))
    
    t1Mb=dplyr::select(maps_1Mb, c(avg_rec,
                                   avg_mut,
                                   exonic_sites,
                                   B,
                                   pi_neu_sim))
    mat=Hmisc::rcorr(as.matrix(t1Mb), type="spearman")
    tbl_1Mb <- mat$r
    tbl_1Mb[upper.tri(tbl_1Mb)] <- mat$P[upper.tri(mat$P)]
    suppressMessages(fwrite(tbl_1Mb, paste("model_", m, "/rep_", r, 
    ...
    # tables for storing correlations with observed pi 
    ...
    # 10 kb
    m_10kb <- lm(pi_exp ~ (B + avg_mut) ^ 2, std_10kb)
    summary(m_10kb)
