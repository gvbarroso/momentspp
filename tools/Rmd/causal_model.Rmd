...
maps_1Mb$dat_carlson <- as.integer(!is.na(maps_1Mb_f$ismc_c))
...
10 kb
```{r SEM-human-maps-fw-10kb, include=F, message=F, warning=F}
...
for(u in unique(human_maps_10kb$mut_map)) {
  ...
  for(m in unique(human_maps_10kb$constrained_model)) { 
    for(d in unique(human_maps_10kb$demo)) {
      ...
      fwrite(anova(m2gls, m3gls),
             paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_10kb_genomewide_",
                   d, "_", m, "_", u, ".csv", sep=""))
      ...
      for(c in 1:22) {
        ...
        fwrite(anova(m2gls, m3gls),
             paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_10kb_chr",
                   c, "_", d, "_", m, "_", u, ".csv", sep=""))
      }
    }
  }
...
100 kb
```{r SEM-human-maps-fw-100kb, include=F, message=F, warning=F}
...
for(u in unique(human_maps_100kb$mut_map)) {
  ...
  for(m in unique(human_maps_100kb$constrained_model)) { 
    for(d in unique(human_maps_100kb$demo)) {
      ...
      fwrite(anova(m2gls, m3gls),
             paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_100kb_genomewide_", 
                   d, "_", m, "_", u, ".csv", sep=""))
      ...
      for(c in 1:22) {
        ...
        fwrite(anova(m2gls, m3gls),
             paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_100kb_chr",
                   c, "_", d, "_", m, "_", u, ".csv", sep=""))
      }
    }
  }
...
1 Mb
```{r SEM-human-maps-fw-1Mb, include=F, message=F, warning=F}
...
for(u in unique(human_maps_1Mb$mut_map)) {
  ...
  for(m in unique(human_maps_1Mb$constrained_model)) { 
    for(d in unique(human_maps_1Mb$demo)) {
      ...
      fwrite(anova(m2gls, m3gls),
             paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_1Mb_genomewide_", 
                   d, "_", m, "_", u, ".csv", sep=""))
      ...
      for(c in 1:22) {
        ...
        fwrite(anova(m2gls, m3gls),
               paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_1Mb_chr",
                     c, "_", d, "_", m, "_", u, ".csv", sep=""))
      }
    }
  }
...
# (Nested) model selection, DAGs 2 and 3 (ANOVA)
# (DAG 3 adds a covariance term between constrained sites and mutation rates)
anova_gw_10kb <- data.frame()
for(u in unique(human_maps_10kb$mut_map)) {
  for(m in unique(human_maps_10kb$constrained_model)) { 
    for(d in unique(human_maps_10kb$demo)) {
      
      tmp <- fread(paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_10kb_genomewide_",
                   d, "_", m, "_", u, ".csv", sep=""))
      ...
      anova_gw_10kb <- rbind.data.frame(anova_gw_10kb, df)
    }
  }
}
anova_gw_10kb$scale <- 1e+4

anova_gw_100kb <- data.frame()
for(u in unique(human_maps_100kb$mut_map)) {
  for(m in unique(human_maps_100kb$constrained_model)) { 
    for(d in unique(human_maps_100kb$demo)) {
      
      tmp <- fread(paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_100kb_genomewide_",
                   d, "_", m, "_", u, ".csv", sep=""))
      ...
      anova_gw_100kb <- rbind.data.frame(anova_gw_100kb, df)
    }
  }
}
anova_gw_100kb$scale <- 1e+5

anova_gw_1Mb <- data.frame()
for(u in unique(human_maps_1Mb$mut_map)) {
  for(m in unique(human_maps_1Mb$constrained_model)) { 
    for(d in unique(human_maps_1Mb$demo)) {
      
      tmp <- fread(paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_1Mb_genomewide_",
                   d, "_", m, "_", u, ".csv", sep=""))
      ...
      anova_gw_1Mb <- rbind.data.frame(anova_gw_1Mb, df)
    }
  }
}
anova_gw_1Mb$scale <- 1e+6

gw_anova <- rbind.data.frame(anova_gw_10kb,
                             anova_gw_100kb,
                             anova_gw_1Mb)

names(gw_anova)[1] <- "pvalue"

p <- ggplot(data=gw_anova, 
             aes(x=mut_map, 
                 y=pvalue, 
                 color=constrained_model,
                 shape=demo)) +
      ...
      scale_color_manual(values=c("purple3", "seagreen4", "brown1"),
                         name="Const. Model") +
      labs(title="ANOVA test of nested DAGs 2 and 3 (genome-wide)",
           x="Mut. Map", y="p-value") +
      theme(axis.title=element_text(size=16), 
            axis.text=element_text(size=12), 
            axis.text.x=element_text(size=12),
            ...
            legend.title=element_text(size=16),
            legend.position="bottom")
save_plot("real_data_maps/lavaan_out/gw_anova.png", p, 
          base_height=8, base_width=15)

# per chr models
## 10 kb
chr_anova_10kb <- data.frame()
for(u in unique(human_maps_10kb$mut_map)) {
  for(m in unique(human_maps_10kb$constrained_model)) { 
    for(d in unique(human_maps_10kb$demo)) {
      chr_list <- list(length=22)
      for(c in 1:22) {
        ...
        tmp <- fread(paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_10kb_chr",
                           c, "_", d, "_", m, "_", u, ".csv", sep=""))
      ...
      df <- do.call(rbind.data.frame, chr_list)
      chr_anova_10kb <- rbind.data.frame(chr_anova_10kb, df)
    }
  }
}
chr_anova_10kb$scale <- 1e+4

## 100 kb
chr_anova_100kb <- data.frame()
for(u in unique(human_maps_100kb$mut_map)) {
  for(m in unique(human_maps_100kb$constrained_model)) { 
    for(d in unique(human_maps_100kb$demo)) {
      chr_list <- list(length=22)
      for(c in 1:22) {
        ...
        tmp <- fread(paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_100kb_chr",
                           c, "_", d, "_", m, "_", u, ".csv", sep=""))
      ...
      df <- do.call(rbind.data.frame, chr_list)
      chr_anova_100kb <- rbind.data.frame(chr_anova_100kb, df)
    }
  }
}
chr_anova_100kb$scale <- 1e+5

## 1 Mb
chr_anova_1Mb <- data.frame()
for(u in unique(human_maps_100kb$mut_map)) {
  for(m in unique(human_maps_100kb$constrained_model)) { 
    for(d in unique(human_maps_100kb$demo)) {
      chr_list <- list(length=22)
      for(c in 1:22) {
        ...
        tmp <- fread(paste("real_data_maps/lavaan_out/ANOVA_DAG2_DAG3_1Mb_chr",
                           c, "_", d, "_", m, "_", u, ".csv", sep=""))
      ...
      df <- do.call(rbind.data.frame, chr_list)
      chr_anova_1Mb <- rbind.data.frame(chr_anova_1Mb, df)
    }
  }
}
chr_anova_1Mb$scale <- 1e+6

chr_anova <- rbind.data.frame(chr_anova_10kb,
                              chr_anova_100kb,
                              chr_anova_1Mb)
names(chr_anova)[1] <- "pvalue"

p <- ggplot(data=filter(chr_anova, demo=="equilibrium"), # eq. =~ 4-epoch
             aes(x=chrom, 
                 y=-log10(pvalue), 
                 color=mut_map)) +
      geom_point(size=2, shape=1) +
      ...
      scale_color_manual(values=c("purple3", "seagreen4", "brown1"),
                         name="Mut. Map") +
      labs(title="ANOVA test of nested DAGs 2 & 3 (models fit per chromosome)",
           x="Chromosome", y="-log10(p-value)") +
      theme(axis.title=element_text(size=16), 
            axis.text=element_text(size=12), 
            axis.text.x=element_text(size=12),
            ...
            legend.title=element_text(size=16),
            legend.position="bottom")
save_plot("real_data_maps/lavaan_out/chr_anova.png", p, 
          base_height=10, base_width=20)
```
...
mut_1Mb_log <- cbind.data.frame(df_1Mb[,1:3],
                                apply(df_1Mb[,4:ncol(df_1Mb)],
                                      2, log))
mut_measurement_3 <- "
...
rec_maps_1kb <- rec_maps_1kb %>% group_by(chrom) %>% 
                                 mutate(bin_1Mb=chromStart %/% 1e+6)
...
# ismc chr 1, from the book chapter Barroso & Dutheil 2025/6
ismc_chr1 <- fread("~/Data/iSMC/iSMC_Mbuti/Mbuti_chr1.rho.1Mb.bedgraph")
names(ismc_chr1)[4:9] <- c(paste("ismc", 1:4, sep="_"), "ismc_mean", "ismc_joint")

# w/ rec we must filter by hand since there's no "coverage" info in the tables
ismc_missing <-  fread("~/Data/iSMC/iSMC_Mbuti/Mbuti_chr1.missing.prop.1Mb.bedgraph")

ismc_chr1$missing_prop <- ismc_missing$`LP6005441-DNA_A08`
ismc_chr1$ismc_1[ismc_chr1$missing_prop > 0.25] <- NA
ismc_chr1$ismc_2[ismc_chr1$missing_prop > 0.25] <- NA
ismc_chr1$ismc_3[ismc_chr1$missing_prop > 0.25] <- NA
ismc_chr1$ismc_4[ismc_chr1$missing_prop > 0.25] <- NA
ismc_chr1$ismc_mean[ismc_chr1$missing_prop > 0.25] <- NA
ismc_chr1$ismc_joint[ismc_chr1$missing_prop > 0.25] <- NA
...
rec_chr1_log <- cbind.data.frame(rec_chr1_1Mb[,1:3],
                                 apply(rec_chr1_1Mb[,4:ncol(rec_chr1_1Mb)],
                                       2, log))
rec_chr1_log$ismc_latent <- ismc_chr1$ismc_latent
...
```{r SEM-chr1, include=F, message=F, warning=F}

rec_chr1_log$chromEnd[nrow(rec_chr1_log)] <- 249000000
names(mut_chr1_1Mb_log)[which(names(mut_chr1_1Mb_log)=="ismc_latent")] <- "ismc_mut_latent"
names(rec_chr1_log)[which(names(rec_chr1_log)=="ismc_latent")] <- "ismc_rec_latent"

tbl_chr1_1Mb <- left_join(mut_chr1_1Mb_log, rec_chr1_log,
                          by=c("chrom", "chromStart", "chromEnd"))

df <- dplyr::select(tbl_chr1_1Mb, c(exp_pi, 
                                    avg_pi, 
                                    mean_mut_masked_roulette,
                                    mean_pyrho_YRI,
                                    prop_del_sites,
...
names(df)[c(3, 4)] <- c("roulette", "pyrho")
cor.test(tbl_chr1_1Mb$avg_pi, tbl_chr1_1Mb$mut_latent, method="pearson")
cor.test(tbl_chr1_1Mb$avg_pi, tbl_chr1_1Mb$mean_mut_masked_roulette)

ggpairs(df)

model_1 <- "
mu=~mean_mut_masked_roulette + mean_mut_masked_carlson + mean_mut_masked_gnomad
avg_pi ~ start(0.25)*c1*mu
"
...
modificationindices(m1gls)
fitMeasures(m1gls)

model_2 <- "
mu=~mean_mut_masked_roulette + mean_mut_masked_carlson + mean_mut_masked_gnomad
avg_pi ~ start(0.25)*c1*mu
mean_mut_masked_roulette ~~ mean_mut_masked_gnomad
"
...
summary(m2gls)
inspect(m2gls, "r2")
modificationindices(m2gls)

model_3 <- "
mu=~mean_mut_masked_roulette + mean_mut_masked_carlson + mean_mut_masked_gnomad
avg_pi ~ start(0.25)*c1*mu
avg_pi ~ start(0.25)*c2*B
mean_mut_masked_roulette ~~ mean_mut_masked_gnomad
"
...
modificationindices(m4gls)

model_5 <- "
mu=~mean_mut_masked_roulette + mean_mut_masked_carlson + mean_mut_masked_gnomad
rec=~mean_pyrho_YRI + mean_deCODE
avg_pi ~ start(0.25)*c1*mu
avg_pi ~ start(0.25)*c2*B
B ~ start(-0.25)*c3*prop_del_sites
B ~ start(0.25)*c4*rec
B ~ start(-0.25)*c5*mu
mu ~ start(0.25)*c6*rec
mu ~~ prop_del_sites
mean_mut_masked_roulette ~~ mean_mut_masked_gnomad
"
...
parameterestimates(m5gls)
inspect(m1gls, "r2")
modificationindices(m1gls)
summary(m1gls, fit.measures=T)
fitMeasures(m1gls)

###
model_0 <- "
rec=~mean_pyrho_YRI + mean_deCODE + mean_pyrho_CEU
avg_pi ~ start(0.25)*c1*rec
"
...
inspect(m0gls, "r2")
parameterEstimates(m0gls)
modificationindices(m0gls)
fitMeasures(m0gls)
...
rec_maps_1Mb$chrom <- as.character(rec_maps_1Mb$chrom)
for(i in 1:22) {
  rec_maps_1Mb$chrom[rec_maps_1Mb$chrom==paste("chr", i, sep="")] <- i
}
rec_maps_1Mb$chrom <- as.integer(rec_maps_1Mb$chrom)

all_maps_1Mb <- left_join(rec_maps_1Mb, df_1Mb, by=c("chrom",
                                                     "chromStart",
                                                     "chromEnd"))

all_maps_1Mb <- filter(all_maps_1Mb, coverage > 0.75, chrom!=19)

all_maps_chr_log <- cbind.data.frame(all_maps_1Mb[,1:3],
                                     apply(all_maps_1Mb[,4:ncol(all_maps_1Mb)],
                                           2, log))
...
mu=~mean_mut_masked_roulette + mean_mut_masked_carlson + mean_mut_masked_gnomad 
avg_pi ~ start(0.25)*c1*mu
mean_mut_masked_roulette ~~ mean_mut_masked_carlson
mean_mut_masked_gnomad ~~ avg_pi
"
