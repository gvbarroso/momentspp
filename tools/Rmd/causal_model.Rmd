...
https://cran.r-project.org/web/packages/ggdag/vignettes/intro-to-ggdag.html TODO
...
# simplifying by focusing on pi_neu
D6p <- DiagrammeR::grViz("
digraph {
  rankdir=LR; // Left to Right, instead of Top to Bottom
  graph []
  node [shape = box]
...
The DAGs that we will test with the data (based on D6p and D7p).
...
# pi neutral as the "final" outcome variable
D1g <- DAG(
  B~avg_rec,
  B~exonic_sites,
  B~avg_mut,
...
# pi "total" as the "final" outcome variable
# non-mutagenic rec
D3g <- DAG(
  B~avg_rec,
  B~exonic_sites,
...
basis_set <- bs1 # testing non-mutagenic recomb. model
weights <- rep(1, length=length(basis_set))
    
DAG1_c_1kb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))
DAG1_c_10kb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))
DAG1_c_100kb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))
DAG1_c_1Mb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))
...
names(DAG1_c_1kb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
names(DAG1_c_10kb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
names(DAG1_c_100kb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
names(DAG1_c_1Mb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    maps_1kb <- dplyr::filter(maps_1kb, row_number() %% 200 == 0) 
    ...
    DAG1_c_1kb[m, rep] <- combine.test(pvals_1kb, weights, "fisher")
    DAG1_c_10kb[m, rep] <- combine.test(pvals_10kb, weights, "fisher")
    DAG1_c_100kb[m, rep] <- combine.test(pvals_100kb, weights, "fisher")
    DAG1_c_1Mb[m, rep] <- combine.test(pvals_1Mb, weights, "fisher")
  }
}
close(pb)

fwrite(DAG1_c_1kb, "DAG1_c_1kb.csv")
fwrite(DAG1_c_10kb, "DAG1_c_10kb.csv")
fwrite(DAG1_c_100kb, "DAG1_c_100kb.csv")
fwrite(DAG1_c_1Mb, "DAG1_c_1Mb.csv")
```

```{r d-sep-DAG2, include=F, message=F, warning=F}

basis_set <- bs2 # testing non-mutagenic recomb. model
weights <- rep(1, length=length(basis_set))
    
DAG2_c_1kb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))
DAG2_c_10kb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))
DAG2_c_100kb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))
DAG2_c_1Mb <- as.data.frame(matrix(nrow=num_models, ncol=num_reps))

# NOTE: interpreting Fisher's C test is tricky because of auto-correlation
# within each of the genomic maps. Looking at larger scales is more informative
names(DAG2_c_1kb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
names(DAG2_c_10kb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
names(DAG2_c_100kb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
names(DAG2_c_1Mb) <- paste("Fisher_C_rep_", 1:num_reps, sep="")
...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    DAG2_c_1kb[m, rep] <- combine.test(pvals_1kb, weights, "fisher")
    DAG2_c_10kb[m, rep] <- combine.test(pvals_10kb, weights, "fisher")
    DAG2_c_100kb[m, rep] <- combine.test(pvals_100kb, weights, "fisher")
    DAG2_c_1Mb[m, rep] <- combine.test(pvals_1Mb, weights, "fisher")
  }
}
close(pb)

fwrite(DAG2_c_1kb, "DAG2_c_1kb.csv")
fwrite(DAG2_c_10kb, "DAG2_c_10kb.csv")
fwrite(DAG2_c_100kb, "DAG2_c_100kb.csv")
fwrite(DAG2_c_1Mb, "DAG2_c_1Mb.csv")
```

Correlations and linear models fist
...
for(m in 1:num_models) {
  ...
  for(rep in 1:num_reps) {
    ...
    mat=Hmisc::rcorr(as.matrix(filter(maps_1kb, row_number() %% 200 == 0)),
    ...
    mat=Hmisc::rcorr(as.matrix(maps_10kb), type="spearman")
    tbl_10kb <- mat$r
...
# excluding exonic_sites from MVN test and keeping only one pi variable
mvn_1kb <- mvn(data=dplyr::select(filter(maps_1kb, row_number() %% 1000 == 0),
...
mvn_10kb <- mvn(data=dplyr::select(filter(maps_10kb, row_number() %% 50 == 0),
                     c(avg_rec, avg_mut, B, sim_pi)),
                transform="log",
                univariatePlot="histogram",
                multivariatePlot="qq",
...
mvn_100kb <- mvn(data=dplyr::select(filter(maps_100kb, row_number() %% 2 == 0),
                      c(avg_rec, avg_mut, B, sim_pi)),
                 transform="log",
                 univariatePlot="histogram",
                 multivariatePlot="qq",
...
# log-stransforming and standardizing variables
filt_1kb <- filter(maps_1kb, row_number() %% 1000 == 0, sim_pi > 0) # filtering
std_1kb <- as.data.frame(apply(apply(dplyr::select(filt_1kb,
  -c(exonic_sites, pi_del_exp)), 2, log), 2, function(x) (x-mean(x)) / sd(x)))

filt_10kb <- filter(maps_10kb, row_number() %% 50 == 0) # filtering
...
svg(paste("model_", m, "/rep_", rep, "/qq_1kb.svg", sep=""), width=9, height=7)
heplots::cqplot(dplyr::select(std_1kb, c(avg_rec,avg_mut, B, sim_pi)))
dev.off()

svg(paste("model_", m, "/rep_", rep, "/qq_10kb.svg", sep=""), width=9, height=7)
heplots::cqplot(dplyr::select(std_10kb, c(avg_rec,avg_mut, B, sim_pi)))
dev.off()

svg(paste("model_", m, "/rep_", rep, "/qq_100kb.svg", sep=""), width=9, height=7)
heplots::cqplot(dplyr::select(std_100kb, c(avg_rec,avg_mut, B, sim_pi)))
dev.off()

svg(paste("model_", m, "/rep_", rep, "/qq_1Mb.svg", sep=""), width=9, height=7)
heplots::cqplot(dplyr::select(std_1Mb, c(avg_rec,avg_mut, B, sim_pi)))
dev.off()

# pairwise scatterplots
## NOTE: here the downsampling serves to reduce file size!
svg(paste("model_", m, "/rep_", rep, "/scatter_1kb.svg", sep=""), width=9, height=7)
pairs(std_1kb)
dev.off()

svg(paste("model_", m, "/rep_", rep, "/scatter_10kb.svg", sep=""), width=9, height=7)
pairs(std_10kb)
dev.off()

svg(paste("model_", m, "/rep_", rep, "/scatter_100kb.svg", sep=""), width=9, height=7)
pairs(filter(std_100kb, row_number() %% 10 == 0))
dev.off()

svg(paste("model_", m, "/rep_", rep, "/scatter_1Mb.svg", sep=""), width=9, height=7)
pairs(std_1Mb)
dev.off()
...
fwrite(parameterestimates(sem3_100kb, ci=T), 
       paste("model_", m, "/rep_", rep, "/lavaan_est_DAG1_1kb.csv", sep=""))

fwrite(as.data.frame(fitted(sem3_100kb)[[1]]),
       paste("model_", m, "/rep_", rep, "/lavaan_fitted_DAG1_1kb.csv", sep=""))

sem_r2 <- rbind.data.frame(inspect(sem1_1kb, "rsquare"),
...
residuals(sem1_100kb)

inspect(sem3_100kb, "rsquare")
parameterestimates(sem3_100kb, ci=T)
fitted(sem3_100kb)
residuals(sem3_100kb)
...
sem <- psem(
B <- gls(B ~ avg_rec + avg_mut, 
         cor=corAR1(0, ~bin), method = "ML",
         data=std_10kb),

sim_pi <- gls(sim_pi ~ B + avg_mut,
              cor=corAR1(0, ~bin), method = "ML",
              data=std_10kb)
)
