...
uL <- c(1e-9, 3e-8, 6e-9, 1e-8)
r <- round(sort(c(seq_log(from=1e-8, to=1e-3, length.out=30),
                  seq(from=1e-4, to=1e-2, length.out=30), 0)), digits=12)
s <- round(-sort(c(seq_log(from=1e-5, to=1e-4, length.out=20), 
                   seq_log(from=7.5e-5, to=1e-3, length.out=40), 0)), digits=12)
  ...
  for(j in 1:length(lookup_s)) {
    for(k in 1:length(lookup_u)) {
      ...
      for(l in 1:length(fnames)) {
        ...
        order <- as.numeric(strsplit(str_sides[2], "_e_1")[[1]][1])
