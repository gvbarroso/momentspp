...
uL <- c(1e-8)
r <- round(sort(c(seq_log(from=1e-8, to=1e-1, length.out=70), 0)), digits=12)
s <- round(-sort(c(seq_log(from=1e-6, to=4e-3, length.out=70), 0)), digits=12)
  ...
  for(j in 1:length(lookup_s)) {
    for(k in 1:length(lookup_u)) {
      ...
      for(l in 1:length(fnames)) {
        ...
        order <- as.numeric(strsplit(str_sides[2], "_e_1_expectations.txt")[[1]][1])
