...
lookup_u <- round(seq_log(from=4e-9, to=1e-8, length.out=20), digits=10)
lookup_r <- round(sort(c(seq_log(from=1e-8, to=1e-3, length.out=20),
                  seq(from=1e-4, to=1e-2, length.out=20))), digits=10)
lookup_s <- round(-sort(c(seq_log(from=1e-5, to=1e-4, length.out=20), 
...
params <- setDT(crossing(Na, N1, t, lookup_u, lookup_r, lookup_s))
