...
u <- 1e-7 # constant among exons as well as "fake" neutral u used to get B-vals
# rounding is required so that directory names don't diverge due to precision
lookup_r <- c(1e-10, 1e-9, 1e-8, 1e-7, 1e-6, 1e-5) #round(seq_log(from=1e-9, to=1e-2, length.out=300), digits=15)
...
params <- read_csv("params.csv", col_names=T)
...
for(i in 1:length(lookup_r)) {
  ...
  for(j in 1:length(lookup_s)) {
    ...
    for(k in 1:length(fnames)) {
      moms <- read_delim(fnames[k], delim=" ", col_names=F)
      idx <- as.numeric(strsplit(strsplit(fnames[k], "_e_1")[[1]][1], 
                                 "model_")[[1]][2])
      
      hls[idx,] <- t(dplyr::select(filter(moms, X1=="Hl_0_0"), X3))
      hrs[idx,] <- t(dplyr::select(filter(moms, X1=="Hr_0_0"), X3))
