...
knitr::opts_knit$set(root.dir="~/Data/momentspp/paper_1/orders/100k")
```
...
lookup_u <- 1e-8 #round(seq_log(from=4e-9, to=1e-8, length.out=10), digits=10)
lookup_r <- round(sort(c(seq_log(from=1e-8, to=1e-3, length.out=20),
                  seq(from=1e-4, to=1e-2, length.out=20))), digits=10)
lookup_s <- c(-1e-3, -1e-4)
  #round(-sort(c(seq_log(from=1e-5, to=1e-4, length.out=20), 
                  #seq_log(from=7.5e-5, to=1e-3, length.out=60))), digits=10)
