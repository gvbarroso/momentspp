...
for(i in 1:length(lookup_r)) {
  ...
  for(j in 1:length(lookup_s)) {
    fnames <- list.files(paste("eq/r_", lookup_r[i], "/s_", lookup_s[j], sep=""),
...
for(i in 1:num_models) {
  ...
  moms <- read.csv(paste("demo/eq/model_", i, 
          "_O_2_expectations.txt", sep=""), sep=" ", header=F)
...
fwrite(lookup_tbl, "lookup_tbl_eq.csv.gz", compress="gzip") 

for(rec in unique(lookup_tbl$r)) {
  p1 <- ggplot(data=filter(lookup_tbl, r==rec, s==-1e-3), aes(x=Generation, y=B)) + 
  ...
  p2 <- ggplot(data=filter(lookup_tbl, r==rec, s==-1e-4), aes(x=Generation, y=B)) + 
    facet_wrap(~as.factor(N1), ncol=2) +
    geom_point() + theme_bw() + geom_line() + 
    scale_x_continuous(breaks=pretty_breaks()) +
    scale_y_continuous(breaks=pretty_breaks()) + 
  ...
  p3 <- ggplot(data=filter(lookup_tbl, r==rec, s==-1e-5), aes(x=Generation, y=B)) + 
    facet_wrap(~as.factor(N1), ncol=2) +
    geom_point() + theme_bw() + geom_line() + 
    scale_x_continuous(breaks=pretty_breaks()) +
    scale_y_continuous(breaks=pretty_breaks()) + 
    labs(title=NULL, x="Generation", y="B (s=-1e-5)") +
    theme(axis.title=element_text(size=16), 
          axis.text=element_text(size=12), 
          axis.text.x=element_text(size=12), 
          axis.text.y=element_text(size=12))
  
  cp <- plot_grid(p1, p2, p3, ncol=1, align='v')
  save_plot(paste("B_time_r_", rec, ".png", sep=""), cp, base_height=10, base_width=16)
}
