...
for(i in 1:length(lookup_r)) {
  ...
  for(j in 1:length(lookup_s)) {
    fnames <- list.files(paste("r_", lookup_r[i], "/s_", lookup_s[j], sep=""),
                         pattern="*_e_1_hets_time.txt", full.names=TRUE)
...
for(rec in unique(lookup_tbl$r)) {
  ...
  for(N in unique(lookup_tbl$N1)) {
    ...
    p1 <- ggplot(data=filter(lookup_tbl, r==rec, s==-1e-3, N1==N),
                 aes(x=Generation, y=B)) + 
      geom_point() + theme_bw() + geom_line() + 
      #geom_hline(aes(yintercept=B_eq, color="red")) + 
      scale_x_continuous(breaks=pretty_breaks()) +
      scale_y_continuous(breaks=pretty_breaks()) 
      if(c==1) {
        p1 <- p1 + labs(title=paste("N1=", N), x=NULL, y="B (s=-1e-3)") +
  ...
  save_plot(paste("B_time_r_", rec, ".png", sep=""), cpr,
...
p <- ggplot(data=filter(lookup_tbl, r==1e-8, Generation <= 2e+4), aes(x=Generation, y=B, color=as.factor(s))) + 
