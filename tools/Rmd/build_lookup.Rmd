...
n_models <- nrow(params)
t <- unique(params$t)
sampling_times <- seq(from=0, to=t, by=500)
...
# viz
c <- 1
cpn <- list(length=length(unique(lookup_tbl$N1)))
legend <- NULL

for(N in unique(lookup_tbl$N1)) {

  p1 <- ggplot(data=filter(lookup_tbl, s==-1e-3, N1==N),
               aes(x=Generation, y=B, color=as.factor(r))) + 
    theme_bw() + geom_line(linewidth=1.25) + 
    #geom_hline(aes(yintercept=B_eq, color=as.factor(r), linetype="dashed")) + 
    scale_x_continuous(breaks=pretty_breaks()) +
    scale_y_continuous(breaks=pretty_breaks()) 
  if(c==1) {
    p1 <- p1 + labs(title="10X Bottleneck", x=NULL, y="B (s=-1e-3)") +
    ...
    p1 <- p1 + labs(title="10X Expansion", x=NULL, y=NULL) +
  ...
  p2 <- ggplot(data=filter(lookup_tbl, s==-1e-4, N1==N),
               aes(x=Generation, y=B, color=as.factor(r))) + 
    theme_bw() + geom_line(linewidth=1.25) + 
    #geom_hline(aes(yintercept=B_eq, color=as.factor(r), linetype="dashed")) + 
    scale_x_continuous(breaks=pretty_breaks()) +
    scale_y_continuous(breaks=pretty_breaks())
  if(c==1) {
    p2 <- p2 + labs(title=NULL, x=NULL, y="B (s=-1e-4)") +
      theme(axis.title=element_text(size=16), 
            axis.text=element_text(size=12), 
            axis.text.x=element_blank(), 
            axis.text.y=element_blank(),
    ...
    p2 <- p2 + labs(title=NULL, x=NULL, y=NULL) +
      theme(axis.text=element_text(size=12), 
            axis.text.x=element_blank(), 
            axis.text.y=element_blank(),
            legend.position="none")
  ...
  p3 <- ggplot(data=filter(lookup_tbl, s==-1e-5, N1==N),
               aes(x=Generation, y=B, color=as.factor(r))) + 
    theme_bw() + geom_line(linewidth=1.25) + 
    #geom_hline(aes(yintercept=B_eq, color=as.factor(r), linetype="dashed")) + 
    scale_x_continuous(breaks=pretty_breaks()) +
  ...
  if(c==1) {
    p3 <- p3 + labs(title=NULL, x="Generation", y="B (s=-1e-5)") +
      theme(axis.title=element_text(size=16), 
            axis.text=element_text(size=12), 
            axis.text.x=element_text(size=12), 
            axis.text.y=element_blank(),
            legend.position="bottom")
    p3 <- p3 + guides(color=guide_legend(title="r"))
  } else {
    p3 <- p3 + labs(title=NULL, x="Generation", y=NULL) +
      theme(axis.title=element_text(size=16), 
            axis.text=element_text(size=12), 
            axis.text.x=element_text(size=12), 
            axis.text.y=element_blank(),
  ...
  cpn[[c]] <- plot_grid(p1, p2, p3, ncol=1, align='v')
  c <- c + 1
}

cpr <- plot_grid(plotlist=cpn, ncol=2)
cpr
#x <- plot_grid(cpr, legend, ncol=1)
#x
...
for(N in unique(lookup_tbl$N1)) {
  ...
  p1 <- ggplot(data=filter(lookup_tbl, r==rec, s==-1e-3, N1==N),
               aes(x=Generation, y=Hl)) + 
    geom_point() + theme_bw() + geom_line() + 
    scale_x_continuous(breaks=pretty_breaks()) +
    scale_y_continuous(breaks=pretty_breaks()) 
    if(c==1) {
      p1 <- p1 + labs(title=paste("N1=", N), x=NULL, y="Hl (s=-1e-3)") +
                 theme(axis.title=element_text(size=16), 
                       axis.text=element_text(size=12), 
                       axis.text.x=element_blank(), 
      ...
      p1 <- p1 + labs(title=paste("N1=", N), x=NULL, y=NULL) +
                 theme(axis.text=element_text(size=12), 
                       axis.text.x=element_blank(), 
                       axis.text.y=element_text(size=12),
                       legend.position="none")
  ...
  p2 <- ggplot(data=filter(lookup_tbl, r==rec, s==-1e-4, N1==N),
               aes(x=Generation, y=Hl)) + 
    geom_point() + theme_bw() + geom_line() + 
    scale_x_continuous(breaks=pretty_breaks()) +
    scale_y_continuous(breaks=pretty_breaks())
    if(c==1) {
      p2 <- p2 + labs(title=NULL, x=NULL, y="Hl (s=-1e-4)") +
                 theme(axis.title=element_text(size=16), 
                       axis.text=element_text(size=12), 
                       axis.text.x=element_blank(), 
                       axis.text.y=element_text(size=12),
  ...
  p3 <- ggplot(data=filter(lookup_tbl, r==rec, s==-1e-5, N1==N),
               aes(x=Generation, y=Hl)) + 
    geom_point() + theme_bw() + geom_line() + 
    scale_x_continuous(breaks=pretty_breaks()) +
    scale_y_continuous(breaks=pretty_breaks())
  ...
  if(c==1) {
      p3 <- p3 + labs(title=NULL, x="Generation", y="Hl (s=-1e-5)") +
                 theme(axis.title=element_text(size=16), 
                       axis.text=element_text(size=12), 
                       axis.text.x=element_text(size=12), 
                       axis.text.y=element_text(size=12),
...
cpr <- plot_grid(plotlist=cpn, ncol=2)
save_plot(paste("Hl_time_r_", rec, ".png", sep=""), cpr,
