...
core_stats <- c("DD_1_1", "Dr_1_1_(1-2p1)^1", "pi2_1_1_1_1", "Hl_1_1")

# constant Ne
mpp <- numeric()
for(i in 1:n_models) {
  vals <- read.table(paste("neutrality/no_migration/model_", i, "_O_1_expectations.txt", sep=""))
  vals <- subset(vals, V1 %in% core_stats)
  vals <- dplyr::slice(vals, 1:9, 13:18, 10:12) # to match moments.LD output and plot
  mpp <- c(mpp, vals$V3)
}
...
for((i=1;i<=$num_models;i++))
do
  momentspp params=opt.bpp F=model_$i.yaml L=model_$i O=60
done
...
expectation <- numeric()
for(i in 1:n_models) {
  vals <- read.table(paste("selection/single-pop/bench/model_", i, "_O_60_expectations.txt", sep=""))
...
for m in *.yaml
do
  momentspp params=opt.bpp F=$m O=60
done
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # left locus (selected)
    ...
    name <- paste("model_", i, "_l_", j, "_O_60_expectations.txt", sep="")
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # right locus
    ...
    for(k in 1:w) { # left locus
      ...
      name <- paste("model_", i, "_l_", k, "_r_", j, "_O_60_expectations.txt", sep="")
...
```{r, BGS-mutvar-grid, include=F, eval=T, results=F, message=F, warning=F}
...
for(i in 1:length(lookup_r)) {
  for(j in 1:length(lookup_s)) {
    moms <- read.table(paste("~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/r_",
                             lookup_r[i], "/model_", as.integer(c), "_O_60_expectations.txt", sep=""))
...
for((i=1;i<=$num_models;i++))
do
  momentspp params=opt.bpp FILE=model_$i.yaml LABEL=model_$i ORDER=60
done
...
expectation <- numeric()
for(i in 1:n_models) {
  vals <- read.table(paste("selection/no_migration/model_", i, "_O_60_expectations.txt", sep=""))
  vals <- subset(vals, V1 %in% core_stats)
