...
do
  ...
  momentspp params=opt.bpp F=model_$i.yaml L=model_$i O=50;
  momentspp params=opt.bpp F=model_$i.yaml L=model_$i O=75;
  momentspp params=opt.bpp F=model_$i.yaml L=model_$i O=100;
  momentspp params=opt.bpp F=model_$i.yaml L=model_$i O=150;
  momentspp params=opt.bpp F=model_$i.yaml L=model_$i O=200;
  momentspp params=opt.bpp F=model_$i.yaml L=model_$i O=300;
done
```
...
p4 <- ggplot(data=filter(tbl_pp, stats==commom_stats[4],
  ...
  theme(axis.title=element_text(size=12),
        ...
        axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
        legend.position="bottom")
save_plot("hr_ratios.png", p4, base_height=24, base_width=12)
...
p5 <- ggplot(data=filter(tbl_pp, stats==commom_stats[5], 
  ...
  theme(axis.title=element_text(size=12),
        ...
        axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
        legend.position="bottom")
save_plot("pi2_ratios.png", p5, base_height=24, base_width=12)
...
for(n in unique(molten_hr$N)) {
  q <- ggplot(data=filter(molten_hr, N==n, r==1e-6), 
    ...
    theme(axis.title=element_text(size=12),
          ...
          legend.position="bottom")
  q
  save_plot(paste("Hr, N=", n, ", r=1e-6.png", sep=""), q,
