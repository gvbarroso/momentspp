...
for(i in 1:n_models) {
  ...
  for(j in 1:(length(t))) {
    rec_spans <- append(rec_spans, models$exon_lengths[i], after=(t[j]))
  }
  ...
  # TODO pick up here
  t <- unique(c(unique(neut_hits$neut_mut_bin[which(diff(neut_hits$neut_segment)!=0)]), neut_hits$neut_mut_bin[nrow(neut_hits)])) # transition points 
  mmap_neutral$end[t]
