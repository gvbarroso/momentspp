...
num_constrained <- 1e+3 # number of (non-recombining) selected segment in chr

# Gamma dist params
scale_sel <- c(1e-5, 2.5e-5, 5e-5, 1e-4, 2.5e-4, 5e-4)
shape_sel <- c(10, 20)
scales_mu <- c(1e-8)
...
for(i in 1:n_models) {
  ...
  bins_50kb <- rep(5e+4, floor(L / 5e+4))
  bins_50kb <- bind_cols(chr="chr1", dplyr::lag(cumsum(bins_50kb), n=1, default=0), dplyr::lag(cumsum(bins_50kb), n=0, default=0))
  names(bins_50kb) <- c("chr", "start", "end")
  gr_50kb <- makeGRangesFromDataFrame(bins_50kb)
  ...
  hits <- findOverlaps(query=gr_50kb, subject=rec_gr) 
  hit_list <- as.data.frame(hits)
  names(hit_list) <- c("bins_50kb", "rec_bins")
  rmap$rec_bins <- 1:nrow(rmap)
  tmpr <- merge(rmap, hit_list)
  rec_50kb <- tmpr %>% group_by(bins_50kb) %>% mutate(avg_rec=weighted.mean(r, end-start))
  rec_50kb <- rec_50kb %>% distinct(bins_50kb, .keep_all=T)
  
  # binning mutation maps
  hits <- findOverlaps(query=gr_50kb, subject=mut_gr) 
  hit_list <- as.data.frame(hits)
  names(hit_list) <- c("bins_50kb", "mut_bins")
  mmap$mut_bins <- 1:nrow(mmap)
  tmpm <- merge(mmap, hit_list)
  mut_50kb <- tmpm %>% group_by(bins_50kb) %>% mutate(avg_mut=weighted.mean(u, end-start))
  mut_50kb <- mut_50kb %>% distinct(bins_50kb, .keep_all=T)
  
  # binning DFE
  dfe_gr <- makeGRangesFromDataFrame(intervals, keep.extra.columns=T)
  hits <- findOverlaps(query=gr_50kb, subject=dfe_gr) 
  hit_list <- as.data.frame(hits)
  names(hit_list) <- c("bins_50kb", "dfe_bins")
  intervals$dfe_bins <- 1:nrow(intervals)
  tmpd <- merge(intervals, hit_list)
  dfe_50kb <- tmpd %>% group_by(bins_50kb) %>% mutate(avg_s=weighted.mean(s, end-start))
  dfe_50kb <- dfe_50kb %>% distinct(bins_50kb, .keep_all=T)
  
  maps_50kb <- bind_cols(bins_50kb, rec_50kb["avg_rec"], mut_50kb["avg_mut"], dfe_50kb["avg_s"])
  
  neu_pos <- numeric(floor(sum_ncsl/1e+2))
  hr_list <- numeric(floor(sum_ncsl/1e+2))
  ...
  for(j in 1:nrow(neutral_intervals)) { 
    ...
    # we sample one neutral site every 100 bp in non-constrained segments
    for(k in 1:floor(neu_span/1e+2)) { 
      
      focal_ur <- mmap[as.data.frame(findOverlaps(query=mut_gr, subject=makeGRangesFromDataFrame(data.frame(chr="chr1", start=pos, end=pos))))["queryHits"][1,],]["u"]
      ...
      for(l in 1:nrow(smap)) { # for every constrained locus
        ...
        focal_s <- smap$s[l]
        focal_ul <- mmap[as.data.frame(findOverlaps(query=mut_gr, subject=makeGRangesFromDataFrame(data.frame(chr="chr1", start=smap$start[l], end=smap$end[l]))))["queryHits"][1,],]["u"]
        ...
        if(pos < smap$start[l]) { # if focal neutral is to the left of focal constrained
          ...
          tmpr <- filter(tmpr, start >= pos, start < smap$start[l])
          
          s_r_intersect <- intersect(makeGRangesFromDataFrame(tmpr, keep.extra.columns=T), makeGRangesFromDataFrame(smap[1:l,], keep.extra.columns=T))
        ...
        if(total_r < 1e-2) {
          ...
          entry <- filter(lookup_tbl, r==closest_r, s==closest_s, ul==closest_ul, ur==closest_ur)
          B <- B * entry$Hr / entry$pi0
