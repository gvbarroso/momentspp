...
R dataframes in future python chunks.
...
for i in range(1, n_models):
  with open(f"selection/single-pop/moments_twoLocus_model_{i}.csv", "w+") as fout:
      n_samples = 40 # 80
...
expectation <- numeric()
for(i in 1:n_models) {
  vals <- read.table(paste("selection/single-pop/moments_twoLocus_model_", i, ".csv", sep=""))$V3
  expectation <- c(expectation, vals)
}

tbl_py <- as.data.frame(expectation)
...
tbl_pp <- df %>% mutate(ratio = expectation / tbl_py$expectation)

p1 <- ggplot(data=tbl_pp[tbl_pp$stats==commom_stats[1],], aes(x=r, y=ratio, shape=N)) 
...
p2 <- ggplot(data=tbl_pp[tbl_pp$stats==commom_stats[2],], aes(x=r, y=ratio, shape=N)) 
...
p3 <- ggplot(data=tbl_pp[tbl_pp$stats==commom_stats[3],], aes(x=r, y=ratio, shape=N)) 
...
p4 <- ggplot(data=tbl_pp[tbl_pp$stats==commom_stats[4],], aes(x=r, y=ratio, shape=N)) 
p4 <- p4 + geom_point(size=3) + geom_line() + theme_bw() + facet_wrap(~s, nrow=1)
p4 <- p4 + scale_shape_manual(values=c(0, 1))
...
p5 <- ggplot(data=tbl_pp[tbl_pp$stats==commom_stats[5],], aes(x=r, y=ratio, shape=N)) 
...
moms_comp
save_plot("selection/single-pop/moms_++_vs_py.pdf", moms_comp, base_height=10, base_width=20)
