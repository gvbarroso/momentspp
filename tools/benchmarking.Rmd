...
# visualizing the relative difference in each moment
diff <- (filter(full_tbl, method=="moments++") %>% select(common_moments) - 
         filter(full_tbl, method=="moments.LD") %>% select(common_moments)) / 
         filter(full_tbl, method=="moments.LD") %>% select(common_moments) 

diff_tbl <- bind_cols(diff, select(filter(full_tbl, method=="moments++"),
                                   -common_moments)) %>% select(-c("method",
                                                                   "sep_moments"))

dat_diff <- pivot_longer(diff_tbl, cols=all_of(common_moments), names_to="variable") 

if(any(dat_diff$m_01 == 0)) {
  p <- ggplot(data=filter(dat_diff, m_01==0),
              aes(x=variable, y=value, color=as.factor(scenario))) 
  p <- p + geom_point(size=2) + theme_bw() + facet_grid(N_0~r)
  p <- p + labs(title = "Rel. diff. (m_01 == 0) x N_0 (rows) and r (cols)",
...
if(any(dat_diff$r == 0)) {
  p <- ggplot(data=filter(dat_diff, r==0),
               aes(x=variable, y=value, color=as.factor(scenario))) 
  p <- p + geom_point(size=2) + theme_bw() + facet_grid(N_0~m_01)
  p <- p + labs(title = "Rel. diff. (r == 0) x N_0 (rows) and m_01 (cols)",
                x="Moment", y="Value", color="Scenario")
  p <- p + theme(axis.title=element_text(size=12),
                 axis.text=element_text(size=10),
                 axis.text.x=element_text(angle=90, size=8, vjust=0.5, hjust=1.0),
