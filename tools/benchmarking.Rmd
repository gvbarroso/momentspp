...
params <- setDT(crossing(N, lookup_r, lookup_ul, lookup_ur, lookup_s))
params$scenario <- 1:nrow(params)
n_models <- nrow(params)
...
for(i in 1:n_models) {
  ...
  setattr(dt_exons, "sorted", "start")
  setattr(dt_exons, "sorted", "end")
  ...
  # finding the cumulative recombination map over the useful intervals (for loop with dplyr::filter seems to be best!)
  ...
  # TODO check warnings in this loop:
  ...
  dfe_bins <- tmpd %>% group_by(bins) %>% mutate(avg_s=weighted.mean(s, end-start)) # avg_s encompasses density of constrained sites
  ...
  for(j in 1:length(samp_pos)) { 
    ...
    for(k in 1:nrow(dt_exons)) { 
      
      focal_s <- dt_exons[k,]$s
      focal_ul <- dt_exons[k,]$u
      total_r <- abs(pos_dt[pos_dt$position==dt_exons[k,]$start + models$exon_lengths[i] / 2,]$cumrec - pos_dt[pos_dt$position==pos,]$cumrec)

      if(total_r < 1e-3) {
        # TODO implement interpolation
        closest_r <- lookup_r[which.min(abs(lookup_r - total_r))] 
        hr <- lookup_tbl[.(focal_ul, focal_ur, focal_s, closest_r)]$Hr
