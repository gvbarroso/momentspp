...
for(i in 1:n_models) {
  ...
  # updates neutral intervals after matching to the rec bins
  ncsl <- x[-1] 
  # checks visually
  neutral_intervals <- bind_cols(chr="chr1", dplyr::lag(cumsum(ncsl), n=1, default=0), dplyr::lag(cumsum(ncsl), n=0, default=0))
  names(neutral_intervals) <- c("chr", "start", "end")
  ...
  mmap_neutral$start <- mmap_neutral$start + 1 # so that GRanges understands the intervals -> NOT in BEDGRAPH format
  mmap_neutral_gr <- makeGRangesFromDataFrame(mmap_neutral)
  neut_hits <- as.data.frame(findOverlaps(query=mmap_neutral_gr, subject=neutral_intervals_gr))
  names(neut_hits) <- c("neut_mut_bin", "neut_segment")
  
  # TODO pick up here
  x <- which(diff(neut_hits$neut_segment)!=0)]
  t <- c(unique(neut_hits$neut_mut_bin[which(diff(neut_hits$neut_segment)!=0)]), neut_hits$neut_mut_bin[nrow(neut_hits)]) # transition points 
  mmap_neutral$end[t]
  
  # NOTE as was done for rec maps => now adapt mut map instead of ncsl
  #####
  # updates neutral intervals after matching to the rec bins
  ncsl <- x[-1] 
  # checks visually
  ...
  for(j in 1:(length(t))) {
    rec_spans <- append(mut_spans, models$exon_lengths[i], after=(t[j]))
  }
  ...
  mmap <- bind_cols(chr="chr1", dplyr::lag(cumsum(mut_spans_neutral), n=1, default=0), dplyr::lag(cumsum(mut_spans_neutral), n=0, default=0), mus)
  ...
  # some disgusting manipulation of the GR objects
  red <- function(x) {
    for(i in 1:(nrow(x) - 1)) {
      if(x[i + 1, 2] < x[i, 3]) {
        if((x[i, 3] - x[i, 2]) == (jump_length - 1)) {
          x[i, 3] <- x[i + 1, 2] - 1
        }
        else {
          x[i + 1, 2] <- x[i, 3] + 1
  ...
  y <- red(a)
  while(any(y$end < y$start)) {
    y <- y[-which(y$end - y$start < 0),]
    y <- red(y)
  }
  y_gr <- makeGRangesFromDataFrame(y)

  l <- list()
  l[[1]] <- y_gr
  l[[2]] <- gaps(y_gr)
  z_gr <- unlist(as(l, "GRangesList"))
  z_df <- arrange(as.data.frame(z_gr), start)
  z_df$r <- 0
  z_gr <- makeGRangesFromDataFrame(z_df)
  
  hits <- findOverlaps(query=z_gr, subject=rec_gr)
