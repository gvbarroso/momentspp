...
write.csv(params_2_pop, "params_2_pops.csv", sep = ",", quote = F, row.names = F, col.names = T)
```

# Running moments++

```{bash run-moments++, engine.opts='-l'}

# 1 population
input="params_1_pop.csv";
((id=id+1)); # line counter, scenario_id
read "$input" # ignores header 
...
while IFS=, read -r a b c d e f g h i j; do
  momentspp params=opt_1.bpp i=$id epochs=$a N0=$f N1=$g m01=$d m10=$e u=$b r=$c;
  id+=1;
done < "$input"
...
```{python run-momentsLD, eval=FALSE, include=FALSE}
...
steadyStateTable <- as.data.frame(matrix(num_scenarios, ncol = num_stats))
finalMomentsTable <- as.data.frame(matrix(num_scenarios, ncol = num_stats))

#names(steadyStateTable) <- 
#names(finalMomentsTable) <- 

idx = 1
for(i in 1:num_scenarios) {
      
  #s <- t(read.table(paste(paste(", "/steady_state.txt", sep = ""))$V5)
