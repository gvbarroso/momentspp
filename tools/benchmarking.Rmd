...
bgs_plot <- plot_grid(plotlist=plot_list, ncol=1, rel_heights=c(1.15, 1, 1, 1, 1.75))
save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_scaling.pdf", bgs_plot, base_height=12, base_width=16)

# plots of moments++
df <- filter(params, w==100)
c <- 1
plot_list <- list(length=length(unique(df$l)))
for(i in unique(df$l)) {
  p <- ggplot(data=filter(df, l==i), aes(x=s, y=momentspp, color=as.factor(N)))
  p <- p + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
  p <- p + geom_hline(data=filter(df, u==i), aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
  ...
  if(c==1) {
    p <- p + labs(title="Hr moments++, rows = L, cols = r", x="s", y=expression(H[r]))
    p <- p + theme(axis.title=element_text(size=16), 
                   axis.text=element_text(size=12), 
                   strip.text.x=element_text(size=16), 
                   axis.title.x=element_blank(),
...
bgs_plot_2 <- plot_grid(plotlist=plot_list, ncol=1, rel_heights=c(1.15, 1, 1, 1, 1.75))
...
plot_list <- list(length=length(unique(df$l)))
for(i in unique(df$l)) {
  p <- ggplot(data=filter(df, l==i), aes(x=s, y=hk95, color=as.factor(N)))
  p <- p + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
  p <- p + geom_hline(data=filter(molten_windows, u==i), aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
  p <- p + scale_x_continuous(breaks=pretty_breaks())
  p <- p + scale_y_log10(breaks=pretty_breaks(), labels=function(x) format(x, scientific=T))
  
  if(c==1) {
    p <- p + labs(title="Hr HK95, rows = ul, cols = r", x="s", y=expression(H[r]))
    p <- p + theme(axis.title=element_text(size=16), 
                   axis.text=element_text(size=12), 
                   strip.text.x=element_text(size=16), 
                   axis.title.x=element_blank(),
...
```{r, demes-BGS-mult-1, include=F, eval=T, results=F, message=F, warning=F}
...
```{bash mk-mult-dirs, engine.opts='-i', include=F, eval=T, results=F, message=F, warning=F}
...
```{r, demes-BGS-mult-2, include=F, eval=T, results=F, message=F, warning=F}
