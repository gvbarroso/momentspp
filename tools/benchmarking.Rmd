...
for(i in 1:n_models) {
  ...
  seg <- filter(dt_exons, start >= 0, end <= 4e+6)
  ...
  hrmap <- hrmap[1:maps_bins$end[nrow(maps_bins)],]# trims the tail for convenience
  avgs <- hrmap %>% group_by(bin) %>% summarize(avg_pi=mean(Hr), avg_pi0=mean(pi0))
  maps_bins$avg_pi <- avgs$avg_pi
  maps_bins$avg_pi0 <- avgs$avg_pi0
  
  windows$pi0 <- 2 * models$N[i] * windows$mus
  windows$alpha <- alpha
  
  fwrite(windows, paste("selection/single-pop/mut_rate_var/model_", i, "/_windows.csv", sep=""))
