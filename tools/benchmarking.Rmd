...
shapes_mu <- c(1, 3, 10, 50, 1000000)
# rec rates
scales_rec <- c(1e-4, 1e-5, 1e-6, 1e-7, 1e-8, 0)
shapes_rec <- c(1, 3, 10, 1000000)
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # right locus
    ...
    for(k in 1:w) { # left locus
      ...
      name <- paste("model_", i, "_l_", k, "_r_", j, "_expectations.txt", sep="")
      expectations <- read.table(paste("selection/single-pop/mut_rate_var/model_", i, "/", name, sep=""))
  ...
  save_plot(paste("selection/single-pop/mut_rate_var/model_", i, "/model_", i, "_landscapes.pdf", sep=""), comb, base_height=10, base_width=12)
  ...
  r2[i, 1] <- (anova.pi$VarExp[1] + anova.pi$VarExp[2] + anova.pi$VarExp[3] + anova.pi$VarExp[4] + anova.pi$VarExp[5] + anova.pi$VarExp[6]) * 100
  r2[i, 2] <- anova.pi$VarExp[1] * 100
  r2[i, 3] <- anova.pi$VarExp[2] * 100
  r2[i, 4] <- anova.pi$VarExp[3] * 100
  r2[i, 5] <- anova.pi$VarExp[3] * 100
  r2[i, 6] <- anova.pi$VarExp[3] * 100
}

summaries <- bind_cols(means_mu, vars_mu, means_rec, vars_rec, means_sel, vars_sel)
names(summaries) <- c("means_mu", "vars_mu", "means_rec", "vars_rec", "means_sel", "vars_sel")
names(landscapes) <- 1:ncol(landscapes)

tbl <- bind_cols(params, summaries, r2, landscapes)
tbl$scale_sel <- -tbl$scale_sel
molten_windows <- pivot_longer(tbl, cols=(ncol(params) + ncol(summaries) + ncol(r2) + 1):ncol(tbl), names_to="window")
molten_windows$window <- as.numeric(molten_windows$window)
...
p1 <- ggplot(data=filter(molten_windows, N==1000, shapes_mu==1e+6, scales_mu==1e-9), aes(x=window, y=value, shape=variable, color=as.factor(scale_sel)))
...
p1 <- p1 + theme(axis.title=element_text(size=16),
               ...
               axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
               legend.position="bottom")
save_plot("selection/single-pop/Hr_landscapes_17.pdf", p1, base_height=10, base_width=15)
...
p2 <- p2 + theme(axis.title=element_text(size=16),
               ...
               axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
               legend.position="bottom")
save_plot("selection/single-pop/Hr_landscapes_18.pdf", p2, base_height=10, base_width=15)
...
p3 <- p3 + theme(axis.title=element_text(size=16),
               ...
               axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
               legend.position="bottom")
save_plot("selection/single-pop/Hr_landscapes_19.pdf", p3, base_height=10, base_width=15)
...
p4 <- p4 + theme(axis.title=element_text(size=16),
               ...
               axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
               legend.position="bottom")
save_plot("selection/single-pop/Hr_landscapes_20.pdf", p4, base_height=10, base_width=15)

# visualizing R^2
molten_tbl <- pivot_longer(tbl, cols=c("Total_R2", "Mutation_R2", "Recombination_R2", "Selection_R2"), names_to="variable")
...
p5 <- p5 + scale_shape_manual(values=c(0, 1, 2, 8), name = "",
                              labels=c(expression(mu), expression(r), expression(alpha), "Total"))
...
p6 <- p6 + scale_shape_manual(values=c(0, 1, 2, 8), name = "",
                              labels=c(expression(mu), expression(r), expression(alpha), "Total"))
...
p7 <- p7 + scale_shape_manual(values=c(0, 1, 2, 8), name = "",
                              labels=c(expression(mu), expression(r), expression(alpha), "Total"))
...
p8 <- p8 + scale_shape_manual(values=c(0, 1, 2, 8), name = "",
                              labels=c(expression(mu), expression(r), expression(alpha), "Total"))
...
# TODO
# plot R^2 mut as a function of the variance of the Gamma distribution of mu,
# with facet_grid(Var(rec)~M), shape = N, color = mean(s)
