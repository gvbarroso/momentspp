...
for(i in 1:n_models) {
  ...
  pl <- pl + scale_y_log10()
  pl <- pl + labs(title="B-values Left", x=NULL, y="B")
  ...
  ppi <- ggplot(data=molten_div[1:5e+5,], aes(x=Pos, y=value, color=Var))
  ppi <- ppi + geom_line() + theme_bw()
  ppi <- ppi + scale_y_log10()
  ppi <- ppi + labs(title=paste("Window of model", i), x="Pos", y="Diversity")
