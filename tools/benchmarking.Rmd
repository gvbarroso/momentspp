...
for(i in 1:n_models) {
  ...
  ss <- -rgamma(n=w, shape=params$shape_sel[i], scale=params$scale_sel[i])
      
  if(colSums(windows[1:100,])["recs"] > 1e-2) { stop("Rec rate is too high!") }
  
  windows <- bind_cols(mus, rs, ss, .name_repair="unique")
  names(windows) <- c("mus", "recs", "dfe")
  windows$pi0 <- 2 * params$N[i] * windows$mus
  windows$alpha <- 2 * params$N[i] * windows$dfe
  
  if(any(windows$alpha < -20)) { stop("Selection is too strong!") }
