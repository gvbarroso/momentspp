...
for(i in 1:n_models) {
  ...
  neutral <- filter(intervals[,2:4], s==0)
  ...
  Rprof() # profiler
  for(j in 1:length(samp_pos)) { # TODO parallelize here
    ...
    for(k in 1:nrow(exons)) { 
      ...
      focal_s <- exons[k,]$s
      focal_ul <- exons[k,]$u
      total_r <- abs(pos_dt[pos_dt$position==exons[k,]$start + 50,]$cumrec - pos_dt[pos_dt$position==pos,]$cumrec)
