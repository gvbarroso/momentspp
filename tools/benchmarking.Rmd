...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_1.pdf", bgs_plot, base_height=12, base_width=16)
...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_2.pdf", bgs_plot_2, base_height=12, base_width=15)
...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_3.pdf", p7, base_height=12, base_width=20)
...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_4.pdf", p8, base_height=12, base_width=20)
...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_5.pdf", p9, base_height=12, base_width=20)
...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_6.pdf", p1, base_height=12, base_width=20)
...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_7.pdf", p2, base_height=12, base_width=20)
...
save_plot("selection/single-pop/simple_bgs/single_neutral/bgs_8.pdf", p3, base_height=12, base_width=20)
```
...
# TODO add change in N over history
N <- c(1000, 10000)
# number of independent genomic windows, each corresponding to a two-locus system
w <- 100
# length of each genomic block
...
# Gamma dist params
# DFE
scale_sel <- c(0.0001, 0.0005)
shape_sel <- 20
# mut rates
scales_mu <- c(1e-8, 1e-9)
shapes_mu <- c(1, 3, 10, 1000000)
# rec rates
scales_rec <- c(1e-6, 1e-7, 1e-8, 1e-9)
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # left locus
    ...
    for(k in 1:w) { # right locus
      ...
      name <- paste("model_", i, "_l_", j, "_r_", k, sep="")
      sink(paste("selection/single-pop/mut_rate_var/", name, ".yaml", sep=""))
...
r2 <- as.data.frame(matrix(nrow = n_models, ncol = 4))
colnames(r2) <- c("Total", "Mutation", "Recombination", "Selection")
...
for(i in 1:n_models) {
  ...
  m.pi.1 <- lm(ss_pi_BGS ~ ss_mus + ss_recs + ss_dfe, data = windows)
  ...
  r2[i, 1] <- (anova.pi$VarExp[1] + anova.pi$VarExp[2] + anova.pi$VarExp[3] + anova.pi$VarExp[4]) * 100
  r2[i, 2] <- anova.pi$VarExp[1] * 100
  r2[i, 3] <- anova.pi$VarExp[2] * 100
  r2[i, 4] <- anova.pi$VarExp[3] * 100
}

tbl <- bind_cols(params, r2)
molten_tbl <- pivot_longer(tbl, cols=c("Total", "Mutation", "Recombination", "Selection"), names_to="variable")

r2_plot <- ggplot(data = molten_tbl, aes(x = bin.size, y = value, shape = variable))
r2_plot <- r2_plot + geom_line(data = molten_tbl) + facet_grid(rho ~ mu_block) 
r2_plot <- r2_plot + geom_point(aes(shape = variable), size = 4)
r2_plot <- r2_plot + scale_shape_manual(values = c(0, 1, 2, 8), name = "",
                                        labels = c("Total", expression(mu), expression(r), expression(alpha)))
r2_plot <- r2_plot + scale_x_continuous(breaks = c(50, 200, 1000), limits = c(40, 1200), trans="log10") 
r2_plot <- r2_plot + scale_y_continuous(breaks = pretty_breaks())
r2_plot <- r2_plot + labs(title = NULL, x = "Bin Size (kb)", y = "Variance Explained (%)") + theme_bw()
r2_plot <- r2_plot + theme(axis.title = element_text(size = 16), axis.text = element_text(size = 12), 
