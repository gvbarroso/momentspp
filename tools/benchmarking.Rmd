...
scales_rec <- c(1e-7) # scaled up by 10x for a 10 Mb genome
...
for(i in 1:n_models) {
  ...
  save_plot(paste("~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/Bval_bench_m_", as.integer(i), ".png", sep=""), cp, base_height=10, base_width=12)
  ...
  seg <- filter(dt_exons, start >= 0, end <= 1e+7)
  ...
  save_plot(paste("~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/pi_m", i, ".pdf", sep=""), ppi2, base_height=10, base_width=12) 
  ...
  lin_models <- list(length=2)
  lin_models[[1]] <- m.pi.1 <- lm(std_pi ~ std_mu + std_r + std_s + std_r:std_s, data=maps_1kb)
  lin_models[[2]] <- lm(std_pi ~ std_mu + std_r, data=maps_1kb)
  
  summary(lin_models[[1]])
  summary(lin_models[[2]])
  
  comp <- AIC(m.pi.1, m.pi.2) # model selection
  
  anova.pi <- Anova(lin_models[[which.min(comp$AIC)]])
  ...
  # 10 kb
  lmpi1 <- lm(std_pi ~ std_mu + std_r + std_s + std_r:std_s, data=maps_10kb)
  lmpi2 <- lm(std_pi ~ std_mu + std_r, data=maps_10kb)
  
  summary(lmpi1)
  summary(lmpi2)
  
  AIC(lmpi1, lmpi2) # model comparison
  
  anova.pi <- Anova(lmpi1)
  apiss <- anova.pi$"Sum Sq"
  anova.pi$VarExp <- apiss / sum(apiss)
  ...
  # 100 kb
  lmpi1 <- lm(std_pi ~ std_mu + std_r + std_s + std_r:std_s, data=maps_100kb)
  lmpi2 <- lm(std_pi ~ std_mu + std_r, data=maps_100kb)
  ...
  save_plot(paste("~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/m", i, "_maps.png", sep=""), lands_scales, base_height=12, base_width=16)
}

fwrite(r2_tbl_1kb, "~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/r2_1kb.csv")
fwrite(r2_tbl_10kb, "~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/r2_10kb.csv")
fwrite(r2_tbl_100kb, "~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/r2_100kb.csv")
