...
write.table(as.data.frame(lookup_r), "selection/single-pop/landscapes/r.csv", quote=F, row.names=F, col.names=F)
write.table(as.data.frame(lookup_ul), "selection/single-pop/landscapes/ul.csv", quote=F, row.names=F, col.names=F)
write.table(as.data.frame(lookup_ur), "selection/single-pop/landscapes/ur.csv", quote=F, row.names=F, col.names=F)
write.table(as.data.frame(lookup_s), "selection/single-pop/landscapes/s.csv", quote=F, row.names=F, col.names=F)

write.csv(params, "selection/single-pop/landscapes/params.csv", quote=F, row.names=F)
...
num_constrained <- 100 # number of (non-recombining) selected segment in chr
...
avg_constrained_lengths <- c(1e+3, 1e+4) 
avg_neutral_lengths <- 1e+5
  
models <- crossing(N, num_constrained, shape_sel, scale_sel, shapes_mu, scales_mu, shapes_rec, scales_rec, 
                   avg_rec_spans, avg_mut_spans, avg_constrained_lengths, avg_neutral_lengths)
...
jump_length <- 1e+3
# TODO make this a function of jump_length
# we build chromosomes of length ~100 Mb (neutral sites + constrained sites)
for(i in 1:n_models) {

  cat(paste("Building chromosome(s) for model ", i, "...\n", sep=""))
  
  csl <- rgeom(n=models$num_constrained[i], prob=1/models$avg_constrained_lengths[i])
  sum_csl <- sum(csl)
  ...
  mus <- rgamma(n=length(mut_spans), shape=models$shapes_mu[i], scale=models$scales_mu[i]/models$shapes_mu[i])
  rs <- rgamma(n=length(rec_spans), shape=models$shapes_rec[i], scale=models$scales_rec[i]/models$shapes_rec[i])
  ss <- -rgamma(n=models$num_constrained[i], shape=models$shape_sel[i], scale=models$scale_sel[i]/models$shape_sel[i])
  ...
  # discretizes values to match lookup table precisely
  ...
  constraints <- c(rbind(rep(0, models$num_constrained[i]), ss))
  ...
  # for binning maps, eg 1 Mb windows
  bins <- rep(1e+6, floor(L / 1e+6))
  ...
  rmap <- bind_cols(chr="chr1", dplyr::lag(cumsum(rec_spans), n=1, default=0), dplyr::lag(cumsum(rec_spans), n=0, default=0), rs)
  ...
  hits <- as.data.frame(findOverlaps(query=intervals_gr, subject=rec_gr))
  gene_rec_bins <- filter(hits, queryHits %% 2 == 0) 
  rmap[unique(gene_rec_bins$subjectHits),]$r <- 0 # TODO fix this: assigns zero rec rate to "genic" regions (with some splash around them) 
  rec_gr <- makeGRangesFromDataFrame(rmap, keep.extra.columns=T) # reset
  ...
  mmap <- bind_cols(chr="chr1", dplyr::lag(cumsum(mut_spans), n=1, default=0), dplyr::lag(cumsum(mut_spans), n=0, default=0), mus)
