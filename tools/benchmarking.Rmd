...
N <- 10000
r <- seq_log(from=1e-15, to=1e-2, length.out=200)
ul <- seq_log(from=1e-9, to=1e-7, length.out=30)
ur <- seq_log(from=1e-9, to=1e-7, length.out=30)
s <- -seq_log(from=1e-5, to=1e-3, length.out=100)

params <- crossing(N, r, ul, ur, s)
params$scenario <- 1:nrow(params)
n_models <- nrow(params)

write.table(as.data.frame(r), "selection/single-pop/landscapes/r.csv", quote=F, row.names=F, col.names=F)
write.table(as.data.frame(ul), "selection/single-pop/landscapes/ul.csv", quote=F, row.names=F, col.names=F)
write.table(as.data.frame(ur), "selection/single-pop/landscapes/ur.csv", quote=F, row.names=F, col.names=F)
write.table(as.data.frame(s), "selection/single-pop/landscapes/s.csv", quote=F, row.names=F, col.names=F)
...
for(i in 1:n_models) {
  ...
  # 
  #
  # TODO: make all spans multiples of 100 or 1000?
  #
  #
  ...
  selected_intervals <- filter(intervals, s < 0)
  ...
  # TODO bin DFE landscape as well
  ...
  maps_50kb <- bind_cols(bins_50kb, rec_50kb["avg_rec"], mut_50kb["avg_mut"])
  
  neu_pos <- numeric(floor(sum_ncsl/1e+3))
  hr <- numeric(floor(sum_ncsl/1e+3))

  pos <- 1 # focal position in chr ---> TODO start at zero?
  for(j in 1:nrow(neutral_intervals)) { 
    
    neu_span <- neutral_intervals$end[j] - neutral_intervals$start[j]
    
    # we sample one neutral site every 1 kp in non-constrained segments
    for(k in 1:ceiling(neu_span/1e+3)) { 
      ...
      pi0 <- 4 * N * as.numeric(focal_ur)
      one_minus_f <- 1
      
      rem_left <- pos - neutral_intervals$start[j]
      rem_right <- neutral_intervals$end[j] - pos
      
      for(l in 1:nrow(selected_intervals)) { # for every constrained locus
      
        s <- selected_intervals$s[l]
        
        focal_ul <- mmap[as.data.frame(findOverlaps(query=mut_gr, subject=makeGRangesFromDataFrame(data.frame(chr="chr1", start=selected_intervals$start[l],  end=selected_intervals$end[l]))))["queryHits"][1,],]["u"]
        
        total_r <- # TODO
          
        hr <- # get from lookup table ~ r, s, ul, ur
        one_minus_f <- one_minus_f * (hr / pi0) ^ csl[l]
      }
      
      hr_BGS[i] <- pi0 * one_minus_f
