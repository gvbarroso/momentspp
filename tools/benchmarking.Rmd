...
```{python run-momentsLD, message=FALSE, warning=FALSE}
...
with open("moments_LD_1_pop_out.csv", "w+") as fout:
    ...
    for p_list in params[1:]:
        N = eval(p_list[p_names.index('N')])
        u = eval(p_list[p_names.index('mu')])
        r = eval(p_list[p_names.index('r')])
        rho = 4 * N * r
        theta = 4 * N * u
        y = moments.LD.Demographics1D.snm(rho, theta)
...
# one epoch plot
p1 <- ggplot(data=filter(dat_stats, num_epochs==1), aes(x=variable, y=value, shape=as.factor(r))) 
...
# two epochs plot
p2 <- ggplot(data=filter(dat_stats, num_epochs==2), aes(x=variable, y=value, shape=as.factor(r)))
...
py_moments <- read.csv("moments_LD_out.csv") # output file from python chunk above

common_moments <- intersect(names(momsppTable), names(py_moments))
match_moments <- bind_rows(select(momsppTable, all_of(common_moments)), select(py_moments, all_of(common_moments)))
...
for(i in 1:length(pop_scenarios)) {
  stats <- read.table(paste("pops_", pop_scenarios[i], "/final_moments.txt", sep = ""))
  dir <- read.table(paste("pops_", pop_scenarios[i], "/timing_direct.txt", sep = ""))
  eigen <- read.table(paste("pops_", pop_scenarios[i], "/timing_eigen.txt", sep = ""))
  pseudo <- read.table(paste("pops_", pop_scenarios[i], "/timing_pseudo-eigen.txt", sep = ""))
