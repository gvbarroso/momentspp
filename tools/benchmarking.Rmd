...
for(i in 1:n_models) {
  ...
  t <- c(unique(neut_hits$neut_rec_bin[which(diff(neut_hits$neut_segment)!=0)]), neut_hits$neut_rec_bin[nrow(neut_hits)]) # transition points of neutral segments within neutral rec map
  ...
  mut_spans_constrained <- rec_spans_constrained
  ...
  mu_l <- rgamma(n=length(mut_spans_constrained), shape=models$shapes_mu[i], scale=models$scales_mu[i]/models$shapes_mu[i])
  ...
  constraints <- c(rbind(rep(0, length(ncsl)), disc_ss))
