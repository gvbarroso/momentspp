...
save_plot("selection/single-pop/bench/moms_++_vs_py_ratios.pdf", moms_comp_ratios, base_height=10, base_width=20)
...
p6 <- p6 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p6
save_plot("selection/single-pop/bench/moms_++_vs_py_dd.pdf", p6, base_height=10, base_width=20)
...
p7 <- p7 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p7
save_plot("selection/single-pop/bench/moms_++_vs_py_dz.pdf", p7, base_height=10, base_width=20)
...
p8 <- p8 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p8
save_plot("selection/single-pop/bench/moms_++_vs_py_hl.pdf", p8, base_height=10, base_width=20)
...
p9 <- p9 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p9
save_plot("selection/single-pop/bench/moms_++_vs_py_hr.pdf", p9, base_height=10, base_width=20)
...
p10 <- p10 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p10
save_plot("selection/single-pop/bench/moms_++_vs_py_pi2.pdf", p10, base_height=10, base_width=20)
...
for(i in unique(params$u)) { # TODO
  p <- ggplot()  
}

p1 <- ggplot(data=filter(params, u==1e-9), aes(x=s, y=hr_BGS, color=as.factor(N)))
p1 <- p1 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p1 <- p1 + geom_hline(data=filter(params, u==1e-9), aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
...
p2 <- ggplot(data=filter(params, u==1e-8), aes(x=s, y=hr_BGS, color=as.factor(N)))
p2 <- p2 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p2 <- p2 + geom_hline(data=filter(params, u==1e-8), aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
...
```{r, demes-BGS-mult, include=F, eval=T, results=F, message=F, warning=F}
...
```{bash run-moments++BGS-mult, engine.opts='-i', include=F, eval=T, results=F, message=F, warning=F}
...
```{r, plots-BGS-mult, include=T, eval=T, results=F, message=F, warning=F}
...
p1 <- p1 + geom_hline(yintercept=filter(molten_windows, u==1e-9)["pi0"], linetype="dashed")
...
p2 <- p2 + geom_hline(yintercept=filter(molten_windows, u==1e-8)["pi0"], linetype="dashed")
...
p3 <- p3 + geom_hline(yintercept=filter(molten_windows, u==1e-7)["pi0"], linetype="dashed")
...
```{r, BGS-mutvar-params, include=F, eval=T, results=F, message=F, warning=F}
...
# Gamma dist params
# DFE
scale_sel <- c(5e-6, 5e-5, 5e-4)
shape_sel <- 20
# mut rates
scales_mu <- c(1e-7, 1e-8, 1e-9)
shapes_mu <- c(1, 10, 30, 100, 300, 1000000)
# rec rates
...
```{bash make_dirs_mutvar, engine.opts='-i', include=F, eval=T, results=F, message=F, warning=F}
...
```{r, BGS-mutvar-demes, include=F, eval=T, results=F, message=F, warning=F}
...
```{bash run-moments++BGS-mutvar, engine.opts='-i', include=F, eval=T, results=F, message=F, warning=F}
...
```{r, plots-BGS-mutvar, include=T, eval=T, results=F, message=F, warning=F}
...
```{r, params-BGS-mutvar-2, include=F, eval=T, results=F, message=F, warning=F}
...
```{bash make_dirs_mutvar_2, eval=T, message=FALSE, warning=FALSE, engine.opts='-i', include=F, results=F}
...
```{r, BGS-mutvar-demes-2, include=F, eval=T, results=F, message=F, warning=F}
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # left locus
    ...
    for(k in 1:w) { # right locus
      ...
      mu_ratio <- l * windows$mus[j] / params$scales_mu[k] # left / right
      total_r <- colSums(windows[j:k,])["recs"]
...
```{bash run-moments++BGS-mutvar-2, engine.opts='-i', include=F, eval=T, results=F, message=F, warning=F}
...
```{r, plots-BGS-mutvar-2, include=T, eval=T, results=F, message=F, warning=F}
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # right locus
    ...
    pi0 <- 2 * params$N[i] * params$mu_r[i]
    one_minus_f <- numeric(length=w)
      
    for(k in 1:w) { # left locus
      
      name <- paste("model_", i, "_l_", k, "_r_", j, "_expectations.txt", sep="")
      expectations <- read.table(paste("selection/single-pop/mut_rate_var_left/model_", i, "/", name, sep=""))
