...
p2 <- p2 + scale_y_log10(breaks=c(0.01, 0.05, 0.10, 0.2, 0.3))
...
p3 <- plot_ly(x=fst$alpha_1, y=fst$alpha_2, z=fst$expectation, type="scatter3d", mode="markers", color=fst$expectation)
p3
export(p, file = "selection/no_migration/Fst_no-mig_left.pdf")
...
p4 <- ggplot(data=fst, aes(x=alpha_1, y=expectation, color=as.factor(r)))
p4 <- p4 + geom_point(size=3) + geom_line() + theme_bw()
p4 <- p4 + scale_color_brewer(palette="PuOr")
p4 <- p4 + scale_y_log10(breaks=pretty_breaks())
p4 <- p4 + labs(title="Symmetric Population Sizes and Selection coefficients", x="2Ns", y="Fst neutral locus")
p4 <- p4 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
p4
save_plot("selection/no_migration/Fst_no-mig_right_1.pdf", p4, base_height=12, base_width=12)
