...
with open("accuracy/moments_LD_out.csv", "w+") as fout:
    ...
    for p_list in params[1:]:
        ...
        N0 = eval(p_list[p_names.index('N_0_epoch_1')])
        N1 = eval(p_list[p_names.index('N_1_epoch_1')])
        rho = 4 * N0 * r
        theta = 4 * N0 * u
        nus = [N0 / N0, N1 / N0]
        ...
        if num_epochs == 2:
            ...
            N2b = eval(p_list[p_names.index('N_2_epoch_2')])
            nus = [N1b / N1, N2b / N1]
...
params_2_pop <- read.csv("params_2_pops.csv", header = T, sep = ",", row.names = F)
...
write.table(stats, "accuracy/moments_2_pops.csv", sep = ",", row.names = F, quote = F)
