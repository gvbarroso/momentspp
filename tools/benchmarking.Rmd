...
for(i in 1:n_models) {
  ...
  fwrite(intervals, "intervals_map.csv")
  
  neutral <- filter(intervals, s==0)
  dt_neutral <- data.table(neutral_intervals)
  setattr(dt_neutral, "sorted", "start")
  setattr(dt_neutral, "sorted", "end")
  
  exons <- filter(intervals, s < 0)
  exons$u <- 1e-8
  exons_dt <- data.table(exons)
  setattr(exons_dt, "sorted", "start")
  setattr(exons_dt, "sorted", "end")
  
  fwrite(rmap, "exons_map.csv")
  ...
  rmap <- bind_cols(chr="chr1", dplyr::lag(cumsum(rec_spans), n=1, default=0), dplyr::lag(cumsum(rec_spans), n=0, default=0))
  ...
  mmap <- bind_cols(chr="chr1", dplyr::lag(cumsum(mut_spans), n=1, default=0), dplyr::lag(cumsum(mut_spans), n=0, default=0))
