...
params_1-pop <- crossing(Ne, mu, r)

write.csv(params_1-pop, "params_1-pop.csv", quote = F, sep = ",", row.names = F)
```
...
params_2-pop <- crossing(mu, r, num_epochs, m_01, m_10, Ne_0, Ne_1)
params_2-pop <- mutate(params_2-pop, Ne_0_epoch_2 = case_when(num_epochs == 1 ~ NA_real_, num_epochs == 2 ~ (Ne_0 / 10)))
params_2-pop <- mutate(params_2-pop, Ne_1_epoch_2 = case_when(num_epochs == 1 ~ NA_real_, num_epochs == 2 ~ (Ne_1 / 10)))
params_2-pop <- mutate(params_2-pop, num_gen_epoch_2 = case_when(num_epochs == 1 ~ NA_real_, num_epochs == 2 ~ 5000))

write.csv(params_2-pop, "params_2-pops.csv", sep = ",", quote = F, row.names = F, col.names = T)
```
...
with open("params_2-pops.csv") as fin:
...
with open("accuracy/moments_LD_out.csv", "w+") as fout:
    ...
    for p_list in params[1:]:
        ...
        N0 = eval(p_list[p_names.index('Ne_0_epoch_1')])
        N1 = eval(p_list[p_names.index('Ne_1_epoch_1')])
        rho = 4 * N0 * r
        theta = 4 * N0 * u
        nus = [N0 / N0, N1 / N0]
        ...
        if num_epochs == 2:
            ...
            N2b = eval(p_list[p_names.index('Ne_2_epoch_2')])
            nus = [N1b / N1, N2b / N1]
...
# Accuracy in 1-pop scenarios

```{r, message=FALSE, warning=FALSE}

params_1-pop <- read.csv("params_1-pop.csv", header = T, sep = ",", row.names = F)
...
stats <- bind_cols(bind_rows(steadyStateTable, finalMomentsTable), bind_rows(params_1-pop, params_1-pop))
stats$type <- c(rep("steady", num_scenarios), rep("final", num_scenarios))

write.table(stats, "accuracy/bench_1-pop_moments.csv", sep = ",", row.names = F, quote = F)
...
# Accuracy in 2-pops scenarios

## moments++ 

```{r, message=FALSE, warning=FALSE}

params_2-pop <- read.csv("params_2-pops.csv", header = T, sep = ",", row.names = F)
num_scenarios <- nrow(params_2-pop)
...
write.table(stats, "accuracy/moments_2-pops.csv", sep = ",", row.names = F, quote = F)
...
full_tbl <- bind_cols(match_moments, bind_rows(params_2-pop, params_2-pop)) # binds parameters together with moments
