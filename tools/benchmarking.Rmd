...
avg_neutral_lengths <- c(3e+3, 1e+4)
...
fwrite(models, "~/Devel/momentspp/benchmarking/selection/single-pop/landscapes/models.csv", row.names=F, quote=F, sep=",")
...
for(i in 1:n_models) {
  ...
  # pre-compute recombination distances between sampled neutral sites and exons, taking into account strength of selection
  ...
  getB <- function(focal_exon, x) { # x is the index of focal sampled site
    ...
    return((hr / (2 * N * u)) ^ models$exon_lengths[i])
  }
  ...
  Bl400 <- unlist(lapply(ex400[[1]], getB, x=1))
  Bq400 <- unlist(lapply(ex400[[length(samp_pos)/4]], getB, x=length(samp_pos)/4))
  Bm400 <- unlist(lapply(ex400[[length(samp_pos)/2]], getB, x=length(samp_pos)/2))
  
  Blr <- unlist(lapply(exons_per_samp_neut[[1]], getB, x=1))
  Bqr <- unlist(lapply(exons_per_samp_neut[[length(samp_pos)/4]], getB, x=length(samp_pos)/4))
  Bmr <- unlist(lapply(exons_per_samp_neut[[length(samp_pos)/2]], getB, x=length(samp_pos)/2))
  ...
  # here we have to use mmap2 which has closed intervals
  ones <- rep(1, L)
  ...
  seg <- filter(exons, start > 1.5e+6, end < 2e+6)
  ...
  ppi <- ppi + geom_line() + theme_bw()
  ppi <- ppi + scale_y_log10()
  ppi <- ppi + labs(title=paste("Window of model", i), x=NULL, y="Diversity")
  ppi <- ppi + theme(axis.title=element_text(size=16),
                     axis.text=element_text(size=12),
                     axis.text.x=element_blank(),
                     legend.position="none")
  a <- plot_grid(ppi, exons_barcode, ncol=1, align='v', axis='l', rel_heights=c(1, 0.1))
  
  #hrmap_j500 <- fread("selection/single-pop/test_data/hrmap_m2_j_500.csv")
  #hrmap_j1000 <- fread("selection/single-pop/test_data/hrmap_m2_j_1000.csv")
  #hrmap_j2000 <- fread("selection/single-pop/test_data/hrmap_m2_j_2000.csv")
  
  #hrj <- cbind.data.frame(hrmap_j500, hrmap_j1000$Hr, hrmap_j2000$Hr)
  #names(hrj) <- c("pos", "j500", "j1000", "j2000")
  #molten_hrj <- pivot_longer(hrj, cols=starts_with("j"), names_to="jump_length")  
  
  # plot comparing B-values obtained with cubic splines interpolation for different sampling densities
  #csip <- ggplot(data=molten_hrj[1:5e+3,], aes(x=pos, y=value, color=jump_length))
  #csip <- csip + geom_line() + theme_bw()
  #csip <- csip + scale_y_log10()
  #csip <- csip + labs(title="Hr interpolation ~ sampling density", x="pos", y="Hr")
  #csip <- csip + theme(axis.title=element_text(size=16),
  #                 axis.text=element_text(size=12),
  #                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
  #                 legend.position="bottom")
