...
```{python run-momentsLD-1pop, include=FALSE}
...
```{r 1-population-plots, message=FALSE, warning=FALSE}
...
p <- ggplot(data=dat_matched, aes(x=variable, y=value, 
                                  shape=method, color=as.factor(scenario)))
...
```{r 1-population-LDx++B, include=FALSE} 
...
```{r 2-population-plots, message=FALSE, warning=FALSE}
...
```{r 2-population-LDx++A, message=FALSE, warning=FALSE}
...
# NOTE: log10 transform may throw away some Dz stats
p <- ggplot(data=dat_matched,
            aes(x=variable, y=value, shape=method, color=as.factor(scenario)))
p <- p + geom_point(size=3) + theme_bw()
p <- p + scale_shape_manual(values=c(1, 2))
p <- p + labs(title="2-Pops moments++ (triangles) vs moments.LD (circles)",
              x="Moment", y="Value")
p <- p + scale_y_continuous(trans="log10")
p <- p + theme(axis.title=element_text(size=12),
               axis.text=element_text(size=10),
...
# check migration rates
p <- ggplot(data=filter(dat_matched, r > 0),
            aes(x=variable, y=value, shape=method, color=as.factor(N_0))) 
p <- p + geom_point(size=3) + theme_bw() + facet_grid(m_01~m_10)
p <- p + scale_y_continuous(trans="log10")
p <- p + scale_shape_manual(values=c(1, 2))
p <- p + labs(title="Moments m_01 (rows) x m_10 (cols) for r > 0",
              x="Moment", y="Value", shape="Method", color="N_0")
p <- p + theme(axis.title=element_text(size=12),
               axis.text=element_text(size=10), 
               axis.text.x=element_text(angle=90, size=8, vjust=0.5, hjust=1.0),
               legend.position="bottom")
p
ggsave("moments++_vs_moments.LD_2-pops_mig.png",
       p, device="png", width=12, height=12)

p <- ggplot(data=filter(dat_matched, m_01==0),
             aes(x=variable, y=value, shape=method)) 
p <- p + geom_point(size=3) + theme_bw() + facet_grid(N_0~r)
p <- p + scale_y_continuous(trans="log10")
p <- p + scale_shape_manual(values=c(1, 2))
p <- p + labs(title = "Moments (m_01 == 0) x N_0 (rows) and r (cols)",
...
p <- p + theme(axis.title=element_text(size=12),
               ...
               legend.position="bottom")
p
ggsave("moments++_vs_moments.LD_2-pops_m01_zero.png",
       p, device="png", width=12, height=12)
...
p <- p + theme(axis.title=element_text(size=12),
               ...
               legend.position="bottom")
p
ggsave("moments++_vs_moments.LD_2-pops_m01_non-zero.png",
       p, device="png", width=12, height=12)
...
p <- p + theme(axis.title=element_text(size=12),
               ...
               legend.position="bottom")
p
ggsave("moments++_vs_moments.LD_2-pops_r_zero.png",
       p, device="png", width=12, height=12)
...
p <- p + theme(axis.title=element_text(size=12),
               ...
               legend.position="bottom")
p
ggsave("moments++_vs_moments.LD_2-pops_r_non-zero.png",
       p, device="png", width=12, height=12)
```

### for each moment/scenario separately

```{r 2-population-LDx++B, message=FALSE, warning=FALSE}
