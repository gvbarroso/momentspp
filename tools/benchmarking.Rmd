...
N <- 10000
lookup_r <- seq_log(from=1e-15, to=1e-2, length.out=200)
lookup_ul <- seq_log(from=1e-9, to=1e-7, length.out=30)
lookup_ur <- seq_log(from=1e-9, to=1e-7, length.out=30)
lookup_s <- -seq_log(from=1e-5, to=1e-3, length.out=100)

# for discretizing Gamma distributions and make look-up faster
dt_u <- data.table(lookup_ul)
dt_r <- data.table(lookup_r)
dt_s <- data.table(sort(lookup_s))
names(dt_s) <- "lookup_s"

setattr(dt_u, "sorted", "lookup_ul")
setattr(dt_r, "sorted", "lookup_r")
setattr(dt_s, "sorted", "lookup_s")

params <- crossing(N, lookup_r, lookup_ul, lookup_ur, lookup_s)
params$scenario <- 1:nrow(params)
n_models <- nrow(params)
...
Collect table with Hr values on the parameter grid
...
params$pi0 <- 2 * params$N * params$ur
n_models <- nrow(params)
c <- 1
...
lookup_tbl <- fread("~/../../media/gvbarroso/extradrive1/landscapes/lookup_tbl.csv")

# constants
N <- unique(lookup_tbl$N)
num_constrained <- 1100 # number of (non-recombining) selected segment in chr
...
models <- crossing(N, num_constrained, shape_sel, scale_sel, shapes_mu, scales_mu, shapes_rec, scales_rec, 
...
# we build chromosomes of length ~100 Mb (neutral sites + constrained sites)
for(i in 1:n_models) {

  cat(paste("Building chromosome(s) for model ", i, "...\n", sep=""))
  
  ncsl <- rgeom(n=models$num_constrained[i], prob=1/models$avg_neutral_lengths[i])
  sum_ncsl <- sum(ncsl)
  
  # dealing with neutral intervals, rec rates and mut rates
  neutral_intervals <- bind_cols(chr="chr1", dplyr::lag(cumsum(ncsl), n=1, default=0), dplyr::lag(cumsum(ncsl), n=0, default=0))
  names(neutral_intervals) <- c("chr", "start", "end")
  neutral_intervals_gr <- makeGRangesFromDataFrame(neutral_intervals)
  
  # recombination maps
  rec_spans_neutral <- rgeom(n=ceiling(sum_ncsl/models$avg_rec_spans[i]), prob=1/models$avg_rec_spans[i])
  if(sum(rec_spans_neutral) < sum_ncsl) {
    rec_spans_neutral <- c(rec_spans_neutral, sum_ncsl - sum(rec_spans_neutral))
  }
  # TODO deal with case sum(rec_spans_neutral) > sum_ncsl
  rec_spans_neutral <- rec_spans_neutral[-which(rec_spans_neutral==0)]
  
  rmap_neutral <- bind_cols(chr="chr1", dplyr::lag(cumsum(rec_spans_neutral), n=1, default=0), dplyr::lag(cumsum(rec_spans_neutral), n=0, default=0))
  names(rmap_neutral) <- c("chr", "start", "end")
  rmap_neutral$start <- rmap_neutral$start + 1 # so that GRanges understands the intervals -> NOT in BEDGRAPH format
  rmap_neutral_gr <- makeGRangesFromDataFrame(rmap_neutral)
  neut_hits <- as.data.frame(findOverlaps(query=rmap_neutral_gr, subject=neutral_intervals_gr))
  names(neut_hits) <- c("neut_rec_bin", "neut_segment")
  t <- unique(c(unique(neut_hits$neut_rec_bin[which(diff(neut_hits$neut_segment)!=0)]), neut_hits$neut_rec_bin[nrow(neut_hits)])) # transition points
  t <- t[which(diff(t)>1)] # deletes consecutive genes
  x <- dplyr::lag(diff(c(0, rmap_neutral$end[t], rmap_neutral$end[nrow(rmap_neutral)])), n=1, default=0) 
  x <- c(x, rmap_neutral$end[nrow(rmap_neutral)] - sum(x))
  ncsl <- x[-1]  # updates neutral intervals after matching to the rec bins
  # checks visually
  neutral_intervals <- bind_cols(chr="chr1", dplyr::lag(cumsum(ncsl), n=1, default=0), dplyr::lag(cumsum(ncsl), n=0, default=0))
  names(neutral_intervals) <- c("chr", "start", "end")
  neutral_intervals$start <- neutral_intervals$start + 1 
  neutral_intervals_gr <- makeGRangesFromDataFrame(neutral_intervals)
  
  rn <- rgamma(n=length(rec_spans_neutral), shape=models$shapes_rec[i], scale=models$scales_rec[i]/models$shapes_rec[i])
  rc <- rgamma(n=length(ncsl), shape=models$shapes_rec[i], scale=models$scales_rec[i]/models$shapes_rec[i])
  disc_rn <- dt_r[dt_r[J(rn), roll="nearest", which=T]]$lookup_r
  disc_rc <- dt_r[dt_r[J(rc), roll="nearest", which=T]]$lookup_r

  rec_spans <- rec_spans_neutral
  for(j in 0:(length(t)-1)) {
    rec_spans <- append(rec_spans, models$exon_lengths[i], after=(t[j+1]+j))
  }
  
  rs_ext <- rn
  for(j in 1:(length(t))) {
    rs_ext <- append(rs_ext, rc[j], after=(t[j]))
  }
  
  rmap <- bind_cols(chr="chr1", dplyr::lag(cumsum(rec_spans), n=1, default=0), dplyr::lag(cumsum(rec_spans), n=0, default=0), rs_ext)
  names(rmap) <- c("chr", "start", "end", "r")
  ...
  rec_dt <- data.table(rmap)
  setattr(rec_dt, "sorted", "start")
  setattr(rec_dt, "sorted", "end")
  
  fwrite(rmap, "rec_map.csv")
  
  csl <- rep(models$exon_lengths[i], length(ncsl)) # non-coding and coding intervals are interdigitated
  sum_csl <- sum(csl)
  L <- sum_csl + sum_ncsl
  ...
  constraints <- c(rbind(rep(0, length(csl)), disc_ss))
  intervals <- c(rbind(ncsl, csl))
  intervals <- bind_cols(chr="chr1", dplyr::lag(cumsum(intervals), n=1, default=0), dplyr::lag(cumsum(intervals), n=0, default=0))
  ...
  intervals$s <- constraints
  intervals <- intervals[-nrow(intervals),] # deletes last gene because it isn't present in rec map
  intervals_gr <- makeGRangesFromDataFrame(intervals, keep.extra.columns=T)
  
  fwrite(intervals, "regions.csv")
  
  # for convenience when traversing the chr
  smap <- filter(intervals, s<0)
  neutral_intervals <- filter(intervals, s==0)
  neutral_intervals_gr <- makeGRangesFromDataFrame(neutral_intervals)
  dt_neutral_intervals <- data.table(neutral_intervals)
  setattr(dt_neutral_intervals, "sorted", "end")
  
  # mutation maps
  mut_spans_neutral <- rgeom(n=ceiling(sum_ncsl/models$avg_mut_spans[i]), prob=1/models$avg_mut_spans[i])
  if(sum(mut_spans_neutral) < sum_ncsl) {
    mut_spans_neutral <- c(mut_spans_neutral, sum_ncsl-sum(mut_spans_neutral))
  }
  # TODO deal with case sum(mut_spans_neutral) > sum_ncsl
  mut_spans_neutral <- mut_spans_neutral[-which(mut_spans_neutral==0)]
  
  mmap_neutral <- bind_cols(chr="chr1", dplyr::lag(cumsum(mut_spans_neutral), n=1, default=0), dplyr::lag(cumsum(mut_spans_neutral), n=0, default=0))
  names(mmap_neutral) <- c("chr", "start", "end")
  dt_mmap <- data.table(mmap_neutral)
  setattr(dt_mmap, "sorted", "end")
  # adapt mut_spans_neutral to fit neutral intervals (reverse was done for rec map)
  t <- dt_mmap[J(dt_neutral_intervals$end), roll="nearest", which=T]
  dt_mmap$end[t] <- dt_neutral_intervals$end
  
  plot(x=1:100, y=mut_spans_neutral[1:100], type="s")
  lines(x=1:100, y=dt_mmap$end[1:100] - dt_mmap$start[1:100], type="s", lty="dashed", col="red")
  
  ##
  mut_spans_neutral <- dt_mmap$end - dt_mmap$start # TODO fix
  ##  
  
  mu_r <- rgamma(n=length(mut_spans_neutral), shape=models$shapes_mu[i], scale=models$scales_mu[i]/models$shapes_mu[i])
  mu_l <- rgamma(n=length(ncsl), shape=models$shapes_mu[i], scale=models$scales_mu[i]/models$shapes_mu[i])
  
  disc_mu_r <- dt_u[dt_u[J(mu_r), roll="nearest", which=T]]$lookup_ul
  disc_mu_l <- dt_u[dt_u[J(mu_l), roll="nearest", which=T]]$lookup_ul
  
  mut_spans <- mut_spans_neutral
  for(j in 0:(length(t)-1)) {
    mut_spans <- append(mut_spans, models$exon_lengths[i], after=(t[j+1]+j))
  }
  
  mus_ext <- mu_r
  for(j in 1:(length(t))) {
    mus_ext <- append(mus_ext, mu_l[j], after=(t[j]))
  }
  
  mmap <- bind_cols(chr="chr1", dplyr::lag(cumsum(mut_spans), n=1, default=0), dplyr::lag(cumsum(mut_spans), n=0, default=0), mus_ext)
  names(mmap) <- c("chr", "start", "end", "u")
  ...
  mut_dt <- data.table(mmap)
  setattr(mut_dt, "sorted", "start")
  setattr(mut_dt, "sorted", "end")
  
  fwrite(mmap, "mut_map.csv")

  
  # finding the cumulative recombination map over the useful intervals
  samps <- rep(jump_length, floor(L/jump_length))
  samp_pos <- bind_cols(chr="chr1", dplyr::lag(cumsum(samps), n=1, default=0), dplyr::lag(cumsum(samps), n=0, default=0))
  names(samp_pos) <- c("chr", "start", "end")
  samp_pos$start <- samp_pos$start + 1 # so that GRanges understands the intervals -> NOT in BEDGRAPH format
  samp_gr <- makeGRangesFromDataFrame(samp_pos)
  a <- bind_rows(samp_pos, dplyr::select(smap, -s))
  a <- arrange(a, start)
  ...
  pb <- txtProgressBar(min = 0, max = nrow(z_df), style = 3)
  for(focal_window in 1:nrow(z_df)) {
    ...
    if(focal_window %in% lookup_tbl_single[,1]) {
      ...
      z_df$r[focal_window] <- rmap$r[lookup_tbl_single[row, 2]]
    }
    else { # if it falls within multiple recombination rate windows
      rec_bins <- lookup_tbl_multiple[lookup_tbl_multiple$genomic_regions == focal_window, 2]
      z_df$r[focal_window] <- weighted.mean(rmap$r[rec_bins], rmap$end[rec_bins] - rmap$start[rec_bins])
    }
  }
  close(pb)
  z_df <- z_df %>% mutate(total_r=r * (end-start))
  z_df$cumrec <- cumsum(z_df$total_r)
  ...
  # to use in the linear models
  maps_bins <- bind_cols(bins, rec_bins["avg_rec"], mut_bins["avg_mut"], dfe_bins["avg_s"])
  
  # starts loop to get diversity at different positions of the genome
  neu_pos <- numeric(floor(sum_ncsl/jump_length))
  hr_list <- numeric(floor(sum_ncsl/jump_length))

  c <- 1 # indexer for containers neu_pos and hr_list
  pos <- 0 # focal position in chr 
  
  Rprof()
  for(j in 1:nrow(neutral_intervals)) { 
    
    neu_span <- neutral_intervals$end[j] - neutral_intervals$start[j]
    
    # we sample one neutral site every 'jump_length' bp in non-constrained segments
    for(k in 1:floor(neu_span/jump_length)) { 
      
      system.time(focal_ur <- mut_dt[J(pos), roll="nearest"]$u) # buggy
      #system.time(focal_ur <- mmap[as.data.frame(findOverlaps(query=mut_gr, subject=makeGRangesFromDataFrame(data.frame(chr="chr1", start=pos, end=pos))))["queryHits"][1,],]["u"])
      pi0 <- 2 * N * as.numeric(focal_ur)
      B <- 1 
      
      for(l in 1:nrow(smap)) { # for every constrained locus
      
        focal_s <- smap$s[l]
        mbins <- as.data.frame(findOverlaps(query=mut_gr, subject=makeGRangesFromDataFrame(data.frame(chr="chr1", start=smap$start[l], end=smap$end[l]))))
        mbins <- mmap[mbins$queryHits,] %>% mutate(avg_mu=(weighted.mean(u, end-start)))
        focal_ul <- mbins$avg_mu[1]
        
        total_r <- 0
        if(pos < smap$start[l]) { # if focal neutral is to the left of focal constrained
          
          tmpr <- rmap
          system.time(tmpr <- filter(tmpr, end >= pos, start < smap$start[l]))
          tmpr$start[1] <- pos # to start rec integration at pos

          system.time(s_r_intersect <- intersect(makeGRangesFromDataFrame(tmpr, keep.extra.columns=T), makeGRangesFromDataFrame(smap[1:l,], keep.extra.columns=T)))
          sliced_range <- as.data.frame(ranges(s_r_intersect)) 
          tmpr$end[nrow(tmpr)] <- sliced_range$start[nrow(sliced_range)]  # to end rec integration at the start of (non-recombining) gene
          
          tmpr <- tmpr %>% mutate(total_r=(weighted.mean(r, end-start) * (tmpr$end[nrow(tmpr)] - tmpr$start[1]))) # weighted sum
          total_r <- tmpr$total_r[1]
        }
        else {
          
          tmpr <- rmap
          tmpr <- filter(tmpr, start <= pos, end > smap$start[l])
          
          tmpr$end[nrow(tmpr)] <- pos # to start rec integration at pos
          tmpr$start[1] <- smap$start[l] # to end rec integration at the start of (non-recombining) gene
          
          tmpr <- tmpr %>% mutate(total_r=(weighted.mean(r, end-start) * (tmpr$end[nrow(tmpr)] - tmpr$start[1]))) # weighted sum
          total_r <- tmpr$total_r[1]
        }
        
        cat(paste("pos =", pos, "gene =", l, "\t"))
        cat(paste(smap$start[l], smap$end[l], "\t"))
        cat(paste("total_r =", total_r, "\n"))

        if(total_r < 1e-3) {
          
          # searching lookup table for closest match 
          closest_r <- lookup_r[which.min(abs(lookup_r - total_r))]
          closest_ul <- lookup_ul[which.min(abs(lookup_ul - focal_ul))]
          closest_ur <- lookup_ur[which.min(abs(lookup_ur - focal_ur))]
          closest_s <- lookup_s[which.min(abs(lookup_s - focal_s))]
          
          system.time(lookup_tbl %>% filter(., r==closest_r, s==closest_s, ul==closest_ul, ur==closest_ur))
          system.time(entry <- filter(lookup_tbl, r==closest_r, s==closest_s, ul==closest_ul, ur==closest_ur))
          #B <- B * (entry$Hr / entry$pi0) ^ csl[l]
          
          # TODO implement interpolation
          #hr <- filter(lookup_tbl, r==closest_r, s==closest_s, ul==closest_ul, ur==closest_ur)$Hr
          #B <- B * (hr / pi0) ^ csl[l] # use coarse-grained predictions Good et al 2014?
        }
        else {
          break
        }
