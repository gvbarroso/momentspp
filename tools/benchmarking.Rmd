...
for(i in unique(params$u)) {
  p <- ggplot(data=filter(params, u==i), aes(x=s, y=hr_BGS, color=as.factor(N)))
  ...
  p <- p + labs(title=NULL, x="s", y=expression(mu[i]))
...
for(i in unique(params$u)) {
  ...
  p <- p + labs(title="Two-locus Reduction Factors BGS, cols = r", x="s", y=expression(mu[i]))
...
save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_2.pdf", bgs_plot_2, base_height=12, base_width=16)
...
for(i in unique(params$u)) {
  ...
  save_plot(paste("selection/single-pop/simple_bgs/bgs_single_neutral_", c, ".pdf", sep=""), p, base_height=8, base_width=15)
