...
lookup_r <- round(seq_log(from=1e-12, to=1e-3, length.out=500), digits=15)
...
for(i in 1:n_models) {
  ...
  mu_dist <- round(seq_log(from=1e-9, to=1e-7, length.out=100), digits=11)
  ...
  relevant_exons <- apply(prd, 2, function(x) x < 1e-3)
  exons_list <- apply(relevant_exons, 1, function(x) which(x))

  # now the loop over sampled (neutral) sites starts
  BGS <- numeric(length=length(exons_list))
  Rprof() 
  #pb <- txtProgressBar(min=1, max=length(exons_list), style=3)
  system.time({
  for(j in 1:length(samp_pos)) {
    #setTxtProgressBar(pb, j)
    B <- 1 
    for(k in 1:length(exons_list[[j]])) { 
      
      focal_s <- dt_exons[exons_list[[j]][k],]$s
      total_r <- prd[j, exons_list[[j]][k]]
      
      # TODO get 3 closest r's, implement interpolation
      ...
      B <- B * (hr / 2 * N * u) ^ models$exon_lengths[i] 
    }
    
    BGS[j] <-  2 * N * pos_dt[j,]$focal_mu * B
  }})
