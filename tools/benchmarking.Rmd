...
for(i in 1:n_models) {
  ...
  getBvals <- function(idx, exons) { # not used at the moment bc passing the indices is hard
    B <- unlist(lapply(exons, getB, x=idx))
    return(cumprod(B)[length(B)])
  }
  
  # we finally get the Hr values per sampled site!
  ...
  hrmap <- bind_cols(neu_pos, Hr_BGS)
  # TODO bin
