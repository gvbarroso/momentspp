...
for(i in 1:n_models) {
  ...
  neutral <- as.data.frame(filter(intervals[,2:4], s==0))
  ...
  setkey(mut_dt, start, end)
  
  fwrite(mmap, "mut_map.csv")

  # finding the cumulative recombination map over the useful intervals
  samp_pos <- unlist(apply(neutral, 1, function(x) seq(from=x[1], to=x[2], by=jump_length))) + 1
  linear_pos <- sort(c(samp_pos, exons$start + models$exon_lengths[i]/2))
  focal_mu <- numeric(length=length(linear_pos))
  cum_rec <- numeric(length=length(linear_pos))
  focal_mu[1] <- mmap[1,]$u
  cum_rec[1] <- rmap[1,]$r
  pb <- txtProgressBar(min=2, max=length(linear_pos), style=3)
  for(j in 2:length(linear_pos)) {
    setTxtProgressBar(pb, j)
    focal_mu[j] <- filter(mmap, start <= linear_pos[j], end >= linear_pos[j])$u 
    focal_r <- filter(rmap, start <= linear_pos[j], end >= linear_pos[j])$r 
    cum_rec[j] <- cum_rec[j-1] + (linear_pos[j] - linear_pos[j-1]) * focal_r
  }
  close(pb)
  
  samp_map <- bind_cols(linear_pos, focal_mu, cum_rec)
  names(samp_map) <- c("position", "focal_mu", "cumrec")
  ...
  # starts loop to get pi at different positions of the genome
  ...
  for(j in 1:length(samp_pos)) { 
    ...
    pos <- samp_pos[j]
    samp_slice <- filter(samp_map, position==pos)
    focal_ur <- samp_slice$focal_mu
    pi0 <- 2 * N * focal_ur
    B <- 1 
    ...
    for(k in 1:nrow(exons)) { 
      ...
      mbins <- as.data.frame(findOverlaps(query=mut_gr, subject=makeGRangesFromDataFrame(data.frame(chr="chr1", start=exons$start[k], end=exons$end[k])))) 
      mbins <- mmap[mbins$queryHits,] %>% mutate(avg_mu=(weighted.mean(u, end-start)))
      focal_ul <- mbins$avg_mu[1]
      # TODO slow, make faster (data.table bin search):
      total_r <- abs(samp_map[samp_map$position==exons[k,]$start + 50,]$cumrec - samp_map[samp_map$position==pos,]$cumrec)

      #cat(paste("pos =", pos, "exon =", k, "\t"))
      #cat(paste(exons$start[k], exons$end[k], "\t"))
      #cat(paste("total_r =", total_r, "\n"))
      closest_r <- lookup_r[which.min(abs(lookup_r - total_r))] # TODO implement interpolation
      
      if(total_r < 1e-3) {
        #system.time(lookup_tbl %>% filter(., r==closest_r, s==closest_s, ul==focal_ul, ur==focal_ur))
        #system.time(entry <- filter(lookup_tbl, r==closest_r, s==closest_s, ul==focal_ul, ur==focal_ur))
        #B <- B * (entry$Hr / entry$pi0) ^ models$exon_lengths[i]
        
        # TODO implement interpolation
        hr <- filter(lookup_tbl, r==closest_r, s==focal_s, ul==focal_ul, ur==focal_ur)$Hr
        B <- B * (hr / pi0) ^ models$exon_lengths[i] # use coarse-grained predictions Good et al 2014?
      } else {
          break
      }
    }
