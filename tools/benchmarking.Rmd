...
s <- c(-1e-3, -5e-4, -2e-4, -1e-4, -5e-5, -2e-5, -1e-5)
...
bgs_plot_1 <- plot_grid(plotlist=plot_list, ncol=1, rel_heights=c(1.15, 1, 1, 1, 1.75))
save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_mpp_1.pdf", bgs_plot_1, base_height=12, base_width=16)
...
for(i in unique(df$l)) {
  ...
  if(c==1) {
    p <- p + labs(title="Hr HK95, rows = ul, cols = r", x="s", y="HK95")
    p <- p + theme(axis.title=element_text(size=16), 
                   axis.text=element_text(size=12), 
                   strip.text.x=element_text(size=16), 
                   axis.title.x=element_blank(),
...
save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_hk95_1.pdf", bgs_plot_3, base_height=12, base_width=16)
...
for(i in unique(df$N)) {
  ...
  for(j in unique(df$l)) {
    ...
    if(c==1) {
      p <- p + labs(title="Hr methods, rows = ul, cols = r", x="s", y="HK95")
      p <- p + theme(axis.title=element_text(size=16), 
                     axis.text=element_text(size=12), 
                     strip.text.x=element_text(size=16), 
                     axis.title.x=element_blank(),
  ...
  save_plot(paste("selection/single-pop/simple_bgs/bgs_mpp_hk95_N_", i, ".pdf", sep=""), bgs_plot_4, base_height=12, base_width=16)
}
...
p <- p + scale_y_log10(breaks=pretty_breaks(), labels=function(x) format(x, scientific=T))
p <- p + labs(title="Diversity Reduction Factors, L=1", x="r", y=expression(1-f[j]))
p <- p + theme(axis.title=element_text(size=12),
               axis.text=element_text(size=10),
               axis.text.x=element_text(angle=90, size=8, vjust=0.5, hjust=1.0),
               legend.position="bottom")
save_plot("selection/single-pop/simple_bgs/s_vs_r_A.pdf", p, base_height=9, base_width=12)
...
p <- p + scale_y_log10(breaks=pretty_breaks(), labels=function(x) format(x, scientific=T))
p <- p + labs(title="Diversity Reduction Factors, L=1", x="cum. rec. rate.", y=expression(1-f[j]))
p <- p + theme(axis.title=element_text(size=12),
               axis.text=element_text(size=10),
               axis.text.x=element_blank(),
               axis.ticks.x=element_blank(),
               legend.position="bottom")
save_plot("selection/single-pop/simple_bgs/s_vs_r_B.pdf", p, base_height=9, base_width=12)

p <- ggplot(data=filter(df4, l==1), aes(x=rec, y=cum_red, color=as.factor(s), linetype=as.factor(u)))
p <- p + geom_line() + facet_wrap(~N, nrow=2) 
p <- p + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
               panel.background=element_blank(), axis.line=element_line(colour="black"))
p <- p + geom_vline(data=filter(df3, l==1), aes(xintercept=-s, color=as.factor(s)), linetype="dashed")
p <- p + scale_shape_manual(values=c(0, 1))
p <- p + scale_x_continuous(breaks=unique(df4$rec))
p <- p + scale_y_continuous(breaks=pretty_breaks(), labels=function(x) format(x, scientific=T))
p <- p + labs(title="Diversity Reduction Factors", x="cum. rec. rate.", y="cum. red.")
p <- p + theme(axis.title=element_text(size=12),
               axis.text=element_text(size=10),
               axis.text.x=element_blank(),
               axis.ticks.x=element_blank(),
               legend.position="bottom")
save_plot("selection/single-pop/simple_bgs/s_vs_r_C.pdf", p, base_height=9, base_width=12)
```
...
mutation rate changes both mu_left and mu_right
...
```{r, plots-BGS-mutvar, include=T, eval=F, results=F, message=F, warning=F}
...
mutation rate changes ONLY mu_left
...
```{bash make_dirs_mutvar_2, eval=F, message=F, warning=F, engine.opts='-i', include=F, results=F}
...
```{r, BGS-mutvar-demes-2, include=F, eval=F, results=F, message=F, warning=F}
...
```{bash run-moments++BGS-mutvar-2, engine.opts='-i', include=F, eval=F, results=F, message=F, warning=F}
...
```{r, plots-BGS-mutvar-2, include=T, eval=F, results=F, message=F, warning=F}
