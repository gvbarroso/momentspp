...
```{r neutral-bench, include=T, eval=F, results=F, message=F, warning=F}
...
```{bash run-moments++-1-pop, engine.opts='-i', include=F, eval=F, results=F, message=F, warning=F}
...
p6 <- ggplot() + geom_errorbar(data=dd[dd$Method=="twoLocusSim",], aes(x=r, ymax=avg + 2*sd, ymin=avg - 2*sd, color=as.factor(N))) + facet_wrap(~s, nrow=1)
...
p6 <- p6 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p6
save_plot("selection/single-pop/moms_++_vs_py_dd.pdf", p6, base_height=10, base_width=20)

dz <- df_m[df_m$stats==commom_stats[2],]
p7 <- ggplot() + geom_errorbar(data=dz[dz$Method=="twoLocusSim",], aes(x=r, ymax=avg + 2*sd, ymin=avg - 2*sd, color=as.factor(N))) + facet_wrap(~s, nrow=1)
...
p7 <- p7 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p7
save_plot("selection/single-pop/moms_++_vs_py_dz.pdf", p7, base_height=10, base_width=20)

hl <- df_m[df_m$stats==commom_stats[3],]
p8 <- ggplot() + geom_errorbar(data=hl[hl$Method=="twoLocusSim",], aes(x=r, ymax=avg + 2*sd, ymin=avg - 2*sd, color=as.factor(N))) + facet_wrap(~s, nrow=1)
...
p8 <- p8 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p8
save_plot("selection/single-pop/moms_++_vs_py_hl.pdf", p8, base_height=10, base_width=20)

hr <- df_m[df_m$stats==commom_stats[4],]
p9 <- ggplot() + geom_errorbar(data=hr[hr$Method=="twoLocusSim",], aes(x=r, ymax=avg + 2*sd, ymin=avg - 2*sd, color=as.factor(N))) + facet_wrap(~s, nrow=1)
...
p9 <- p9 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p9
save_plot("selection/single-pop/moms_++_vs_py_hr.pdf", p9, base_height=10, base_width=20)

pi2 <- df_m[df_m$stats==commom_stats[5],]
p10 <- ggplot() + geom_errorbar(data=pi2[pi2$Method=="twoLocusSim",], aes(x=r, ymax=avg + 2*sd, ymin=avg - 2*sd, color=as.factor(N))) + facet_wrap(~s, nrow=1)
...
p10 <- p10 + theme(axis.title=element_text(size=16),
                 ...
                 legend.position="bottom")
p10
save_plot("selection/single-pop/moms_++_vs_py_pi2.pdf", p10, base_height=10, base_width=20)
...
```{r, demes-BGS, include=F, eval=T, results=F, message=F, warning=F}

N <- c(2500, 10000)
u <- c(1e-7, 1e-8, 1e-9)
r <- c(1e-5, 1e-6, 1e-7, 1e-8, 1e-9)
s <- c(-1e-5, -2.5e-5, -1e-4, -2.5e-4, -1e-3)
...
```{bash run-moments++BGS, engine.opts='-i', include=F, eval=T, results=F, message=F, warning=F}

source ~/.bashrc

cd selection/single-pop/simple_bgs/

for m in *.yaml
do
  momentspp params=opt.bpp F=$m O=20
done
...
```{r, plots-BGS, include=T, eval=T, results=F, message=F, warning=F}
...
# for landscape plots
factors_tbl <- as.data.frame(matrix(nrow=n_models, ncol=100))
...
for(i in 1:n_models) {
  ...
  factors_tbl[i,] <- one_minus_f
}

params$factors <- factors
params$hr_BGS <- hr_BGS

p1 <- ggplot(data=params[params$u==1e-9,], aes(x=s, y=hr_BGS, color=as.factor(N)))
p1 <- p1 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p1 <- p1 + geom_hline(data=params[params$u==1e-9,], aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
p1 <- p1 + scale_x_continuous(breaks=pretty_breaks())
p1 <- p1 + scale_y_log10(breaks=c(5e-6, 1e-5, 2e-5), labels=function(x) format(x, scientific=T))
p1 <- p1 + labs(title="Two-locus Hr BGS", x="s", y=expression(mu[1e-9]))
...
p2 <- ggplot(data=params[params$u==1e-8,], aes(x=s, y=hr_BGS, color=as.factor(N)))
p2 <- p2 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p2 <- p2 + geom_hline(data=params[params$u==1e-8,], aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
p2 <- p2 + scale_x_continuous(breaks=pretty_breaks())
p2 <- p2 + scale_y_log10(breaks=c(5e-5, 1e-4, 2e-4), labels=function(x) format(x, scientific=T))
p2 <- p2 + labs(title=NULL, x="s", y=expression(mu=mu[1e-8]))
...
p3 <- ggplot(data=params[params$u==1e-7,], aes(x=s, y=hr_BGS, color=as.factor(N)))
p3 <- p3 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p3 <- p3 + geom_hline(data=params[params$u==1e-7,], aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
p3 <- p3 + scale_x_continuous(breaks=pretty_breaks())
p3 <- p3 + scale_y_log10(breaks=c(5e-4, 1e-3, 2e-3), labels=function(x) format(x, scientific=T))
p3 <- p3 + labs(title=NULL, x="s", y=expression(mu[1e-7]))
...
bgs_plot <- plot_grid(p1, p2, p3, ncol=1, rel_heights=c(1.15, 1, 1.25))
save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_1.pdf", bgs_plot, base_height=12, base_width=16)

p4 <- ggplot(data=params[params$u==1e-9,], aes(x=s, y=factors, color=as.factor(N)))
p4 <- p4 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p4 <- p4 + geom_hline(yintercept=1, linetype="dashed")
p4 <- p4 + scale_x_continuous(breaks=pretty_breaks())
p4 <- p4 + scale_y_log10(breaks=pretty_breaks(), labels=function(x) sprintf("%.3f", x))
p4 <- p4 + labs(title="Two-locus Reduction Factors BGS", x="s", y=expression(mu[1e-9]))
...
p5 <- ggplot(data=params[params$u==1e-8,], aes(x=s, y=factors, color=as.factor(N)))
p5 <- p5 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p5 <- p5 + geom_hline(yintercept=1, linetype="dashed")
p5 <- p5 + scale_x_continuous(breaks=pretty_breaks())
p5 <- p5 + scale_y_log10(breaks=pretty_breaks(), labels=function(x) sprintf("%.3f", x))
p5 <- p5 + labs(title=NULL, x="s", y=expression(mu[1e-8]))
...
p6 <- ggplot(data=params[params$u==1e-7,], aes(x=s, y=factors, color=as.factor(N)))
p6 <- p6 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p6 <- p6 + geom_hline(yintercept=1, linetype="dashed")
p6 <- p6 + scale_x_continuous(breaks=pretty_breaks())
p6 <- p6 + scale_y_log10(breaks=c(0.2, 0.4, 0.6, 0.8, 1), labels=function(x) sprintf("%.3f", x))
p6 <- p6 + labs(title=NULL, x="s", y=expression(mu[1e-7]))
p6 <- p6 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12), 
                 strip.text.x=element_blank(), 
                 strip.text.y=element_text(size=16),
...
bgs_plot_2 <- plot_grid(p4, p5, p6, ncol=1, rel_heights=c(1.15, 1, 1.25))
save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_2.pdf", bgs_plot_2, base_height=12, base_width=15)
...
p7 <- ggplot(data=molten_windows[molten_windows$u==1e-9,], aes(x=window, y=value, color=as.factor(s)))
p7 <- p7 + geom_point(size=1) + facet_grid(N~r) + theme_bw()
p7 <- p7 + scale_x_discrete(breaks=pretty_breaks())
p7 <- p7 + scale_y_continuous(breaks=pretty_breaks())
p7 <- p7 + labs(title="Two-locus Hr BGS windows, mu=1e-9", x="Window", y=expression(1-f[i]))
p7 <- p7 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")

save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_3.pdf", p7, base_height=12, base_width=20)

p8 <- ggplot(data=molten_windows[molten_windows$u==1e-8,], aes(x=window, y=value, color=as.factor(s)))
p8 <- p8 + geom_point(size=1) + facet_grid(N~r) + theme_bw()
p8 <- p8 + scale_x_discrete(breaks=pretty_breaks())
p8 <- p8 + scale_y_continuous(breaks=pretty_breaks())
p8 <- p8 + labs(title="Two-locus Hr BGS windows, mu=1e-8", x="Window", y=expression(1-f[i]))
p8 <- p8 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")

save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_4.pdf", p8, base_height=12, base_width=20)

p9 <- ggplot(data=molten_windows[molten_windows$u==1e-7,], aes(x=window, y=value, color=as.factor(s)))
p9 <- p9 + geom_point(size=1) + facet_grid(N~r) + theme_bw()
p9 <- p9 + scale_x_discrete(breaks=pretty_breaks())
p9 <- p9 + scale_y_continuous(breaks=pretty_breaks())
p9 <- p9 + labs(title="Two-locus Hr BGS windows, mu=1e-7", x="Window", y=expression(1-f[i]))
p9 <- p9 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")

save_plot("selection/single-pop/simple_bgs/bgs_single_neutral_5.pdf", p9, base_height=12, base_width=20)
```

## Multiple Neutral Loci
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # left locus
    ...
    for(k in 1:w) { # right locus
      ...
      sink(paste("selection/single-pop/simple_bgs/mult_neutral/", name, ".yaml", sep=""))
...
for(i in 1:n_models) {
  ...
  for(j in 1:w) { # right locus
    ...
    for(k in 1:w) { # left locus
      ...
      expectations <- read.table(paste("selection/single-pop/simple_bgs/mult_neutral/", name, sep=""))
...
p1 <- ggplot(data=molten_windows[molten_windows$u==1e-9,], aes(x=window, y=value, color=as.factor(alpha)))
...
p1 <- p1 + geom_hline(yintercept=molten_windows[molten_windows$u==1e-9,]$pi0, linetype="dashed")
p1 <- p1 + labs(title="Two-locus Hr BGS windows, mu=1e-9", x="Window", y=expression(H[r]))
p1 <- p1 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")

save_plot("selection/single-pop/simple_bgs/bgs_mult_neutral_1.pdf", p1, base_height=12, base_width=20)

p2 <- ggplot(data=molten_windows[molten_windows$u==1e-8,], aes(x=window, y=value, color=as.factor(alpha)))
...
p2 <- p2 + geom_hline(yintercept=molten_windows[molten_windows$u==1e-8,]$pi0, linetype="dashed")
p2 <- p2 + labs(title="Two-locus Hr BGS windows, mu=1e-8", x="Window", y=expression(H[r]))
p2 <- p2 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")

save_plot("selection/single-pop/simple_bgs/bgs_mult_neutral_2.pdf", p2, base_height=12, base_width=20)

p3 <- ggplot(data=molten_windows[molten_windows$u==1e-7,], aes(x=window, y=value, color=as.factor(alpha)))
...
p3 <- p3 + geom_hline(yintercept=molten_windows[molten_windows$u==1e-7,]$pi0, linetype="dashed")
p3 <- p3 + labs(title="Two-locus Hr BGS windows, mu=1e-7", x="Window", y=expression(H[r]))
p3 <- p3 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")

save_plot("selection/single-pop/simple_bgs/bgs_mult_neutral_3.pdf", p3, base_height=12, base_width=20)
```

# BGS with Mutation rate variation

## Variation in both loci

This first set of models explore the situation where the local
mutation rate changes both mu_l and mu_r 

### Building Demes files

```{r, demes-BGS-1, include=F, eval=T, results=F, message=F, warning=F}
...
# Gamma dist params
# DFE
scale_sel <- c(1e-5, 1e-4, 5e-4)
shape_sel <- 20
# mut rates
scales_mu <- c(1e-7, 1e-8, 1e-9)
shapes_mu <- c(1, 10, 30, 100, 1000000)
# rec rates
scales_rec <- c(1e-4, 1e-5, 1e-6, 1e-7, 1e-8, 1e-9, 0)
shapes_rec <- c(0.1, 1, 10, 1000000)

params <- crossing(N, w, l, shape_sel, scale_sel, shapes_mu, scales_mu, shapes_rec, scales_rec)
params$scenario <- 1:nrow(params)
n_models <- nrow(params)

write.csv(params, "selection/single-pop/mut_rate_var/params.csv", quote=F, row.names=F)

Sys.setenv(num_models=n_models)
Sys.setenv(w=w)
```

```{bash make_dirs, engine.opts='-i', include=F, eval=T, results=F, message=F, warning=F}

source ~/.bashrc

cd selection/single-pop/mut_rate_var/
...
```{r, demes-BGS-2, include=F, eval=T, results=F, message=F, warning=F}

for(i in 1:n_models) {
  
  cat(paste("Writing yaml files for model ", i, "...\n", sep=""))
  
  mus <- rgamma(n=w, shape=params$shapes_mu[i], scale=params$scales_mu[i]/params$shapes_mu[i])
  rs <- rgamma(n=w, shape=params$shapes_rec[i], scale=params$scales_rec[i]/params$shapes_rec[i])
  ss <- -rgamma(n=w, shape=params$shape_sel[i], scale=params$scale_sel[i]/params$shape_sel[i])
  alpha <- 2 * params$N[i] * ss
  ...
  while(sum(rs) > 1e-2) { 
    ...
    cat("Total recombination rate of chr is too high! Redrawing...\n")
    rs <- rgamma(n=w, shape=params$shapes_rec[i], scale=params$scales_rec[i]/params$shapes_rec[i])
  }
  
  windows <- bind_cols(mus, rs, ss, .name_repair="unique")
  names(windows) <- c("mus", "recs", "dfe")
  windows$pi0 <- 2 * params$N[i] * windows$mus
  windows$alpha <- alpha
  ...
  for(j in 1:w) { # left locus
    ...
    for(k in 1:w) { # right locus
      ...
      mu_ratio <- l * windows$mus[j] / windows$mus[k] # left / right
      total_r <- colSums(windows[j:k,])["recs"]
      
      name <- paste("model_", i, "_l_", j, "_r_", k, sep="")
      sink(paste("selection/single-pop/mut_rate_var/model_", i, "/", name, ".yaml", sep=""))
...
params <- read.table("selection/single-pop/mut_rate_var/params.csv", sep=",", header=T)
n_models <- nrow(params)
hr_BGS <- numeric(length=n_models)
landscapes <- as.data.frame(matrix(nrow=n_models, ncol=w))
...
for(i in 1:n_models) {
  ...
  windows <- read.csv(paste("selection/single-pop/mut_rate_var/model_", i, "_windows.csv", sep=""), header=T)
  hr_BGS <- numeric(length=w)
  ...
  for(j in 1:w) { # right locus
    ...
    for(k in 1:w) { # left locus
      ...
      name <- paste("model_", i, "_l_", k, "_r_", j, "_expectations.txt", sep="")
      expectations <- read.table(paste("selection/single-pop/mut_rate_var/", name, sep=""))
      
      hr <- expectations[expectations$V1 == "Hr_0_0",]$V3
      one_minus_f[k] <- hr / pi0
    }
    
    hr_BGS[j] <- pi0 * prod(one_minus_f)
  }
  
  landscapes[i,] <- hr_BGS
  windows$hr_BGS <- hr_BGS

  windows$bin <- 1:nrow(windows)
  molten_bins <- pivot_longer(windows, cols=c("mus", "recs", "alpha", "pi0", "hr_BGS"), names_to="variable")
  
  p1 <- ggplot(data=windows, aes(x=bin, y=hr_BGS))
  p1 <- p1 + geom_line() + theme_bw()
  p1 <- p1 + scale_x_discrete(breaks=pretty_breaks())
  p1 <- p1 + scale_y_continuous(breaks=pretty_breaks())
  p1 <- p1 + labs(title=NULL, x=NULL, y=expression(H[r]))
  p1 <- p1 + theme(axis.title=element_text(size=16),
                   axis.text=element_text(size=12),
  ...
  p2 <- ggplot(data=windows, aes(x=bin, y=mus))
  p2 <- p2 + geom_line() + theme_bw()
  p2 <- p2 + scale_x_discrete(breaks=pretty_breaks())
  p2 <- p2 + scale_y_continuous(breaks=pretty_breaks())
  p2 <- p2 + labs(title=NULL, x=NULL, y=expression(mu))
  p2 <- p2 + theme(axis.title=element_text(size=16),
                   axis.text=element_text(size=12),
                   axis.text.y=element_blank(),
                   axis.ticks=element_blank(),
  ...
  p3 <- ggplot(data=windows, aes(x=bin, y=recs))
  p3 <- p3 + geom_line() + theme_bw()
  p3 <- p3 + scale_x_discrete(breaks=pretty_breaks())
  p3 <- p3 + scale_y_continuous(breaks=pretty_breaks())
  p3 <- p3 + labs(title=NULL, x=NULL, y=expression(r))
  p3 <- p3 + theme(axis.title=element_text(size=16),
                   axis.text=element_text(size=12),
                   axis.text.y=element_blank(),
                   axis.ticks=element_blank(),
  ...
  p4 <- ggplot(data=windows, aes(x=bin, y=alpha))
  p4 <- p4 + geom_line() + theme_bw()
  p4 <- p4 + scale_x_discrete(breaks=pretty_breaks())
  p4 <- p4 + scale_y_continuous(breaks=pretty_breaks())
  p4 <- p4 + labs(title=NULL, x=NULL, y=expression(alpha))
  p4 <- p4 + theme(axis.title=element_text(size=16),
                   axis.text=element_text(size=12),
                   axis.text.y=element_blank(),
                   axis.ticks=element_blank(),
  ...
  save_plot(paste("selection/single-pop/mut_rate_var/model_", i, "_landscapes.pdf", sep=""), comb, base_height=10, base_width=12)
  ...
  for(j in unique(molten_windows$scales_mu)) {
    for(k in unique(molten_windows$scales_rec)) {
      ...
      p <- p + theme(axis.title=element_text(size=16),
                     ...
                     axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                     legend.position="bottom")
      save_plot(paste("selection/single-pop/Hr_landscapes_", c, ".pdf", sep=""), p, base_height=10, base_width=15)
...
p1 <- p1 + theme(axis.title=element_text(size=16),
                 ...
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
save_plot(paste("selection/single-pop/mut_rate_var/Hr_landscapes_", c + 1, ".pdf", sep=""), p1, base_height=10, base_width=15)
...
p2 <- p2 + theme(axis.title=element_text(size=16),
                 ...
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
save_plot(paste("selection/single-pop/mut_rate_var/Hr_landscapes_", c + 2, ".pdf", sep=""), p2, base_height=10, base_width=15)
...
p3 <- p3 + theme(axis.title=element_text(size=16),
                 ...
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
save_plot(paste("selection/single-pop/mut_rate_var/Hr_landscapes_", c + 3, ".pdf", sep=""), p3, base_height=10, base_width=15)
...
p4 <- p4 + theme(axis.title=element_text(size=16),
                 ...
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
save_plot(paste("selection/single-pop/mut_rate_var/Hr_landscapes_", c + 4, ".pdf", sep=""), p4, base_height=10, base_width=15)
...
save_plot("selection/single-pop/R2_plot_1.pdf", p5, base_height=10, base_width=15)
...
p6 <- p6 + theme(axis.title=element_text(size=16), 
                 ...
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position="bottom")
save_plot("selection/single-pop/R2_plot_2.pdf", p6, base_height=10, base_width=15)
...
p7 <- p7 + theme(axis.title=element_text(size=16), 
                 ...
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position="bottom")
save_plot("selection/single-pop/R2_plot_3.pdf", p7, base_height=10, base_width=15)
...
p8 <- p8 + theme(axis.title=element_text(size=16), 
                 ...
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position="bottom")
save_plot("selection/single-pop/R2_plot_4.pdf", p8, base_height=10, base_width=15)
```

## Variation in left locus only

This second set of models explore the situation where the local
mutation rate changes ONLY mu_l
...
scale_sel <- c(1e-5, 1e-4, 5e-4)
shape_sel <- 20
# mut rate right locus
mu_r <- 1e-8
# mut rates left locus
scales_mu <- c(1e-7, 1e-8, 1e-9)
shapes_mu <- c(10, 100, 1000000)
# rec rates
scales_rec <- c(1e-5, 1e-6, 1e-7, 1e-8)
shapes_rec <- 1

params <- crossing(N, w, l, shape_sel, scale_sel, mu_r, shapes_mu, scales_mu, shapes_rec, scales_rec)
params$scenario <- 1:nrow(params)
n_models <- nrow(params)

write.csv(params, "selection/single-pop/mut_rate_var_left/params.csv", quote=F, row.names=F)

Sys.setenv(num_models=n_models)
Sys.setenv(w=w)
```

```{bash make_dirs, eval=T, message=FALSE, warning=FALSE, engine.opts='-i', include=F, results=F}

source ~/.bashrc

cd selection/single-pop/mut_rate_var_left/
...
for(i in 1:n_models) {
  ...
  write.csv(windows, paste("selection/single-pop/mut_rate_var_left/model_", i, "/windows.csv", sep=""), quote=F, row.names=F)
  
  for(j in 1:w) { # left locus
    
    for(k in 1:w) { # right locus
      
      mu_ratio <- l * windows$mus[j] / params$mu_r[k] # left / right
      total_r <- colSums(windows[j:k,])["recs"]
      
      name <- paste("model_", i, "_l_", j, "_r_", k, sep="")
      sink(paste("selection/single-pop/mut_rate_var_left/model_", i, "/", name, ".yaml", sep=""))
...
params <- read.table("selection/single-pop/mut_rate_var_left/params.csv", sep=",", header=T)
n_models <- nrow(params)
hr_BGS <- numeric(length=n_models)
landscapes <- as.data.frame(matrix(nrow=n_models, ncol=w))
...
for(i in 1:n_models) {
  ...
  windows <- read.csv(paste("selection/single-pop/mut_rate_var_left/model_", i, "/windows.csv", sep=""), header=T)
  hr_BGS <- numeric(length=w)
  ...
  for(j in 1:w) { # right locus
    ...
    for(k in 1:w) { # left locus
      ...
      name <- paste("model_", i, "_l_", k, "_r_", j, "_expectations.txt", sep="")
      expectations <- read.table(paste("selection/single-pop/mut_rate_var_left/model", i, "/", name, sep=""))
  ...
  comb <- plot_grid(p1, p2, p3, p4, ncol=1)
  save_plot(paste("selection/single-pop/mut_rate_var_left/model_", i, "/landscapes.pdf", sep=""), comb, base_height=10, base_width=12)
...
tbl <- bind_cols(params, r2, landscapes)
tbl$scale_sel <- -tbl$scale_sel
write.csv(tbl, "selection/single-pop/mut_rate_var_left/tbl.csv", quote=F, row.names=F)
  ...
  for(j in unique(molten_windows$scales_mu)) {
    for(k in unique(molten_windows$scales_rec)) {
      ...
      p <- p + theme(axis.title=element_text(size=16),
                     ...
                     axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                     legend.position="bottom")
      save_plot(paste("selection/single-pop/mut_rate_var_left/Hr_landscapes_", c, ".pdf", sep=""), p, base_height=10, base_width=15)
...
# zooming in plots with tiny mut rate variation
p1 <- ggplot(data=filter(molten_windows,shapes_mu==1e+6, scales_mu==1e-9), aes(x=window, y=value, color=as.factor(scale_sel)))
p1 <- p1 + geom_line() + facet_grid(shapes_rec~scales_rec) + theme_bw()
p1 <- p1 + scale_x_discrete(breaks=pretty_breaks())
p1 <- p1 + scale_y_continuous(breaks=pretty_breaks())
p1 <- p1 + labs(title="Hr landscapes, shape_mu=1e+6, scale_mu=1e-9, rows~1/var_rec, cols~mean_rec", x="Window", y=expression(H[r]))
p1 <- p1 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
save_plot(paste("selection/single-pop/mut_rate_var_left/Hr_landscapes_", c + 1, ".pdf", sep=""), p1, base_height=10, base_width=15)

p2 <- ggplot(data=filter(molten_windows, shapes_mu==1e+6, scales_mu==1e-8), aes(x=window, y=value, color=as.factor(scale_sel)))
p2 <- p2 + geom_line() + facet_grid(shapes_rec~scales_rec) + theme_bw()
p2 <- p2 + scale_x_discrete(breaks=pretty_breaks())
p2 <- p2 + scale_y_continuous(breaks=pretty_breaks())
p2 <- p2 + labs(title="Hr landscapes, shape_mu=1e+6, scale_mu=1e-8, rows~1/var_rec, cols~mean_rec", x="Window", y=expression(H[r]))
p2 <- p2 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
save_plot(paste("selection/single-pop/mut_rate_var_left/Hr_landscapes_", c + 2, ".pdf", sep=""), p2, base_height=10, base_width=15)

p3 <- ggplot(data=filter(molten_windows, shapes_mu==1e+6, scales_mu==1e-7), aes(x=window, y=value, color=as.factor(scale_sel)))
p3 <- p3 + geom_line() + facet_grid(shapes_rec~scales_rec) + theme_bw()
p3 <- p3 + scale_x_discrete(breaks=pretty_breaks())
p3 <- p3 + scale_y_continuous(breaks=pretty_breaks())
p3 <- p3 + labs(title="Hr landscapes, shape_mu=1e+6, scale_mu=1e-7, rows~1/var_rec, cols~mean_rec", x="Window", y=expression(H[r]))
p3 <- p3 + theme(axis.title=element_text(size=16),
                 axis.text=element_text(size=12),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.position="bottom")
save_plot(paste("selection/single-pop/mut_rate_var_left/Hr_landscapes_", c + 3, ".pdf", sep=""), p3, base_height=10, base_width=15)

# visualizing R^2
molten_tbl <- pivot_longer(tbl, cols=c("Total_R2", "Mutation_R2", "Recombination_R2", "Selection_R2", "Mu:Sel_R2", "Rec:Sel_R2"), names_to="variable")

p5 <- ggplot(data=filter(molten_tbl, scales_mu==1e-9), aes(x=shapes_mu, y=value, shape=variable, color=as.factor(scale_sel)))
p5 <- p5 + geom_line() + facet_grid(shapes_rec~scales_rec)+ theme_bw()
p5 <- p5 + geom_point(aes(shape=variable), size=4)
p5 <- p5 + scale_shape_manual(values=c(0, 1, 2, 3, 4, 8), name = "",
                              labels=c(expression(mu:alpha), expression(mu), expression(r:alpha), expression(r), expression(alpha), "Total"))
p5 <- p5 + scale_x_continuous(breaks=unique(molten_tbl$shapes_mu), trans="log10") 
p5 <- p5 + scale_y_continuous(breaks=pretty_breaks())
p5 <- p5 + labs(title = "Var Explained, mu=1e-9, rows~1/var_rec, cols~mean_rec", x="~1/var_mu", y="Variance Explained (%)")
p5 <- p5 + theme(axis.title=element_text(size=16), 
                 axis.text=element_text(size=12), 
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position="bottom")
save_plot("selection/single-pop/mut_rate_var_left/R2_plot_1.pdf", p5, base_height=10, base_width=15)

p6 <- ggplot(data=filter(molten_tbl, scales_mu==1e-8), aes(x=shapes_mu, y=value, shape=variable, color=as.factor(scale_sel)))
p6 <- p6 + geom_line() + facet_grid(shapes_rec~scales_rec)+ theme_bw()
p6 <- p6 + geom_point(aes(shape=variable), size=4)
p6 <- p6 + scale_shape_manual(values=c(0, 1, 2, 3, 4, 8), name = "",
                              labels=c(expression(mu:alpha), expression(mu), expression(r:alpha), expression(r), expression(alpha), "Total"))
p6 <- p6 + scale_x_continuous(breaks=unique(molten_tbl$shapes_mu), trans="log10") 
p6 <- p6 + scale_y_continuous(breaks=pretty_breaks())
p6 <- p6 + labs(title = "Var Explained, mu=1e-8, rows~1/var_rec, cols~mean_rec", x="~1/var_mu", y="Variance Explained (%)")
p6 <- p6 + theme(axis.title=element_text(size=16), 
                 axis.text=element_text(size=12), 
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position="bottom")
save_plot("selection/single-pop/mut_rate_var_left/R2_plot_2.pdf", p6, base_height=10, base_width=15)

p7 <- ggplot(data=filter(molten_tbl, scales_mu==1e-7), aes(x=shapes_mu, y=value, shape=variable, color=as.factor(scale_sel)))
p7 <- p7 + geom_line() + facet_grid(shapes_rec~scales_rec)+ theme_bw()
p7 <- p7 + geom_point(aes(shape=variable), size=4)
p7 <- p7 + scale_shape_manual(values=c(0, 1, 2, 3, 4, 8), name = "",
                              labels=c(expression(mu:alpha), expression(mu), expression(r:alpha), expression(r), expression(alpha),"Total"))
p7 <- p7 + scale_x_continuous(breaks=unique(molten_tbl$shapes_mu), trans="log10") 
p7 <- p7 + scale_y_continuous(breaks=pretty_breaks())
p7 <- p7 + labs(title="Var Explained, mu=1e-7, rows~1/var_rec, cols~mean_rec", x="~1/var_mu", y="Variance Explained (%)")
p7 <- p7 + theme(axis.title=element_text(size=16), 
                 axis.text=element_text(size=12), 
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position="bottom")
save_plot("selection/single-pop/mut_rate_var_left/R2_plot_3.pdf", p7, base_height=10, base_width=15)
```
...
u <- 1e-7
r  <- c(1e-4, 1e-5, 1e-6, 1e-7, 0)
s1 <- c(0, -5e-05, -2.5e-04, -5e-04, -1e-3)
s2 <- c(0, -5e-05, -2.5e-04, -5e-04, -1e-3)
sanc <- c(0, -5e-05, -1e-3)
