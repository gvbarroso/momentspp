...
for(i in 1:n_models) {
  ...
  hr_BGS[i] <- pi0 * prod(one_minus_f)
}

params$hr_BGS <- hr_BGS

p1 <- ggplot(data=params[params$u==1e-8,], aes(x=s, y=hr_BGS, color=as.factor(N)))
p1 <- p1 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p1 <- p1 + geom_hline(data=params[params$u==1e-8,], aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
p1 <- p1 + scale_x_continuous(breaks=pretty_breaks())
p1 <- p1 + scale_y_log10(breaks=c(5e-5, 1e-4, 2e-4), labels=function(x) format(x, scientific=T))
p1 <- p1 + labs(title="Two-locus Hr BGS", x="s", y=expression(mu=1e-8))
...
p2 <- ggplot(data=params[params$u==1e-7,], aes(x=s, y=hr_BGS, color=as.factor(N)))
p2 <- p2 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p2 <- p2 + geom_hline(data=params[params$u==1e-7,], aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
p2 <- p2 + scale_x_continuous(breaks=pretty_breaks())
p2 <- p2 + scale_y_log10(breaks=c(4e-4, 1e-3, 2e-3), labels=function(x) format(x, scientific=T))
p2 <- p2 + labs(title=NULL, x="s", y=expression(mu=1e-7))
...
p3 <- ggplot(data=params[params$u==1e-6,], aes(x=s, y=hr_BGS, color=as.factor(N)))
p3 <- p3 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p3 <- p3 + geom_hline(data=params[params$u==1e-6,], aes(yintercept=pi0, color=as.factor(N)), linetype="dashed")
p3 <- p3 + scale_x_continuous(breaks=pretty_breaks())
p3 <- p3 + scale_y_log10(breaks=c(1e-7, 1e-5, 1e-3, 1e-2), labels=function(x) format(x, scientific=T))
p3 <- p3 + labs(title=NULL, x="s", y=expression(mu=1e-6))
...
p1 <- ggplot(data = params[params$u==1e-8,], aes(x=s, y=hr_BGS, shape=as.factor(l), color=as.factor(N)))
p1 <- p1 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p1 <- p1 + scale_shape_manual(values=c(0, 1))
p1 <- p1 + scale_x_continuous(breaks=unique(params$s))
p1 <- p1 + scale_y_log10(breaks=pretty_breaks())
p1 <- p1 + labs(title="Two-locus BGS u=1e-8", x="s", y=expression(Hr[BGS]))
p1 <- p1 + theme(axis.title = element_text(size = 16), axis.text = element_text(size = 12), 
...
p2 <- ggplot(data = params[params$u==1e-7,], aes(x=s, y=hr_BGS, shape=as.factor(l), color=as.factor(N)))
p2 <- p2 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p2 <- p2 + scale_shape_manual(values=c(0, 1))
p2 <- p2 + scale_x_continuous(breaks=unique(params$s))
p2 <- p2 + scale_y_log10(breaks=pretty_breaks())
p2 <- p2 + labs(title="Two-locus BGS u=1e-7", x="s", y=expression(Hr[BGS]))
p2 <- p2 + theme(axis.title = element_text(size = 16), axis.text = element_text(size = 12), 
                 strip.text.x = element_text(size = 16), strip.text.y = element_text(size = 16),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position = "bottom")

p3 <- ggplot(data=params[params$u==1e-6,], aes(x=s, y=hr_BGS, shape=as.factor(l), color=as.factor(N)))
p3 <- p3 + geom_line() + geom_point(size=3) + facet_wrap(~r, nrow=1) + theme_bw()
p3 <- p3 + scale_shape_manual(values=c(0, 1))
p3 <- p3 + scale_x_continuous(breaks=unique(params$s))
#p3 <- p3 + scale_y_log10(breaks=unique(params[params$u==1e-6,]$))
p3 <- p3 + labs(title="Two-locus BGS, u=1e-6", x="s", y=expression(Hr[BGS]))
p3 <- p3 + theme(axis.title = element_text(size = 16), axis.text = element_text(size = 12), 
                 strip.text.x = element_text(size = 16), strip.text.y = element_text(size = 16),
                 axis.text.x=element_text(angle=90, size=12, vjust=0.5, hjust=1.0),
                 legend.text=element_text(size=16), legend.position = "bottom")
