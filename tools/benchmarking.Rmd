...
for(i in 1:n_models) {
  ...
  N <- as.integer(params[i, 1])
  r <- as.numeric(params[i, 2])
  ul <- as.numeric(params[i, 3])
  ur <- as.numeric(params[i, 4])
  s <- as.numeric(params[i, 5])
  f <- ul / ur
...
shape_sel <- 20
# mut rates
scales_mu <- c(1e-8, 3e-9, 1e-9)
shapes_mu <- c(1, 10, 30, 100, 300, 1e+3, 1e+9)
# rec rates
scales_rec <- c(1e-8)
shapes_rec <- c(0.1, 1, 10, 1e+3, 1e+9)

# TODO correlation between rec and mut

params <- crossing(N, shape_sel, scale_sel, shapes_mu, scales_mu, shapes_rec, scales_rec)
params$scenario <- 1:nrow(params)
n_models <- nrow(params)
