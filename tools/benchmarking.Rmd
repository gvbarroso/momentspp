...
for(i in 1:n_models) {
  ...
  rec_spans <- rgeom(n=ceiling(L/1e+4), prob=1/1e+4)
  if(sum(rec_spans) < L) {
    rec_spans <- c(rec_spans, L-sum(rec_spans))
  }

  mut_spans <- rgeom(n=ceiling(L/1e+4), prob=1/1e+4)
  if(sum(mut_spans) < L) {
  ...
  fwrite(intervals, "regions.bedgraph")
  ...
  rmap$end[nrow(rmap)] <- L # clips leftover span in last bin
  ...
  rmap[unique(gene_rec_bins$subjectHits),]$r <- 0 # assigns zero rec rate to genic regions (with some splash around them)
  ...
  mmap$end[nrow(mmap)] <- L # clips leftover span in last bin
  ...
  x <- bind_rows(samp_pos, dplyr::select(smap, -s)) 
  y <- arrange(x, start)
  # TODO continue here
