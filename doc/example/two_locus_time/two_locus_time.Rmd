---
title: "Example run of moments++, pure two-locus modeol, 10-fold bottleneck"
...
This example run assumes that moments++ package has been installed.
Our goal is to predict B-values over time after a 10-fold bottleneck.
...
cat("    epochs:\n")
cat("      - end_time: 0\n")
cat("        rates: [0]\n") #  we're only interested in pi0 here
cat("  - name: selection\n")
cat("    start_time: .inf\n")
cat("    epochs:\n")
cat("      - end_time: 0\n")
    ...
    do
      ...
      if [[ $(echo "$a < -1" | bc) == 1 ]]
	    then
	      b=${a%.*}
	      Ns=$(($b * -2))
	      # choosing an appropriate order of (1-2p) factors based on Ns
	      if [[ $Ns -lt 30 ]]
	      then
...
```{r, lookup, include=T, eval=T, results=F, message=F, warning=F}
...
fnames <- list.files("output/demo", pattern="*_e_*.*_time.txt", full.names=TRUE)
hets <- NULL
for(l in 1:length(fnames)) {
  moms <- fread(fnames[l])
  str_sides <- strsplit(fnames[l], "_O_")[[1]]
...
# plot raising B-values to 1000th power
